...
- 문서 버전: `E1E6_CANON_v3`
- 작성일: 2026-07-31
- 현재 workstream: `E1–E6 READ-ONLY RESEARCH DESIGN REVIEW`
- 저장소 유효 상태: `E1–E6 DESIGN AMENDED / ALL NEW EXECUTION BLOCKED`
...
[●] P2 E1–E6 연구설계 검토        — READ-ONLY / NO EXECUTION
[ ] P3 E4–E6 Prior 방법 개발·동결  — NOT_STARTED
[ ] P4 최종 실험·저널 작성        — NOT_STARTED
```

Current task: `E1E6_CANON_v3 RESEARCH-DESIGN REVIEW / scientific_verdict null`.

Next: 사용자 검토로 dual-output O/X rubric, change-status rule, numerical margins와
independent test design을 동결한다. 그 전에는 training/Roofer/mesh/viewer 실행을 시작하지 않는다.

Gate S0는 P2의 **entry substate**다. `DEC-P1-013`은 전체 Gate 또는 confirmatory
performance를 승인한 것이 아니라 historical exact development split의 C1/C2 feasibility만
승인했다. 새 E1–E6 execution, validation, held-out과 final acceptance verdict는 차단된다.

> **중요:** 위 패널이 현재 E1–E6 프로그램의 유효 흐름이다. 기존
...
flowchart LR
    ...
    S0 -->|DEC-P1-021 design amendment| P2["P2 E1–E6 research design<br/>READ-ONLY"]
    P2 -->|PASS criterion 동결<br/>사용자 Gate 2| P3["P3 Prior 방법 개발·동결<br/>NOT_STARTED"]
    P3 -->|방법·loss 동결<br/>사용자 Gate 3| P4["P4 held-out test 전 건물 × E1–E6<br/>최종 실험·저널<br/>NOT_STARTED"]
...
동결된 E1–E6 전체 condition matrix를 실행한다는 뜻이다. 같은 건물에서 학습에 쓰지
...
`DEC-P1-021`부터 결과는 두 평행 output chain으로 나뉜다. Roofer/LoD2 chain은 자동
LoD2 범위확대의 confirmatory primary이고, semantic textured mesh chain은 독립 key
secondary다. 두 O/X와 rescue/regression을 합치거나 상호 복사하지 않는다.
...
|---|---|---|---|---|---|---|
| P1 | 무엇을 연구하고 현재 repo/data가 지원하는지 확정 | bootstrap 요청, 현행 정본 확인 | charter, roadmap, novelty, data/result/handoff contracts, audit packet | read-only audit 9종, Return Packet | E1–E6 정본 전환과 audit integration | `COMPLETE` |
| P2 | Gate S0 뒤 E1/E2/E3 기준선과 dual-output acceptance 동결 | `DEC-P1-021`; 새 실행은 별도 승인 전 blocked | E1–E6 protocol, Roofer/mesh OX와 threshold 결정안 | 현재는 설계·동결 문서만 | independent test 결과 전 criterion 동결 | `READ-ONLY DESIGN REVIEW` |
| P3 | 관찰된 실패에 맞춘 prior 방법 개발·동결 | P2 criterion 동결 | loss/method 설계와 ablation 계약 | frozen E3 control 및 E4/E5/E6 구현·ablation, Return Packet | held-out building 접근 전 사용자 최종 방법·schedule 동결 | `NOT_STARTED` |
| P4 | held-out test 전 건물 × E1–E6 최종 실험과 원고 | P3 method freeze, data/reference/split freeze | 결과 해석, manuscript | held-out 전 건물 runs, metrics, case sheets, receipts | 사용자 결론·원고 승인 | `NOT_STARTED` |
...
| P2 | spatial pilot + validation | E1–E3 baseline, adapter, dual-output threshold/criterion 동결 | 접근 금지 |
| P3 | spatial pilot + validation | E3는 동결 control로만 재실행/재사용; E4/E5/E6 개발·ablation·schedule 동결 | 접근 금지 |
| P4 primary | held-out test의 모든 building | 동결된 E1–E6 최종 비교 | 최초 접근 |
| P4 supplementary, sampled fallback일 때 선택 | 모든 사전 등록 `E_pair(m)` 및 가능한 `E_all6` building | primary 잠금 후 census coverage atlas·보조 기술통계 | primary 결론 변경 금지 |
...
1. **P1 audit:** `U_target`, contrast별 `E_pair(m)`과 `E_all6` 후보 수, asset/reference coverage,
   후보 AOI별 공통 footprint·연속성·면적, 공간 group, 결측 사유, E1–E6 예상
...
2. **P2 pre-result Gate S0:** 첫 baseline 결과를 보기 전에 사용자가 다음 중 하나를
   ...
   - `EXHAUSTIVE_PARTITION` — 기본 우선안. 사전 등록한 contrast universe 전 건물을
...
4. **P2/P3:** 동일한 development+validation pool을 사용한다. P3는 development에서
   E4/E5/E6를 개발하고 validation에서 방법을 선택·동결한다. 동결 후 E4/E5/E6를
   development+validation 전 건물에 final coverage run으로 적용한다. E1/E2는
   exact-compatible P2 결과를 재사용하고, E3는 hash가 맞는 frozen result를
   재사용하거나 protocol-matched rerun하여 E1–E6 matrix를 완성한다.
5. **P4:** 사전에 동결된 held-out building 전부에 E1–E6를 실행한다.

`EXHAUSTIVE_PARTITION`이면 P4 자체는 held-out remainder만 실행하지만, P2/P3 결과와
합치면 프로그램 종료 시 각 `E_pair(m)` paired matrix와 가능한 `E_all6` matrix가 완성된다.
`STRATIFIED_SAMPLE`이면 primary inference는 동결 표본에 한정하고, 전수 확장은 P4
primary 잠금 뒤 별도 census로 수행한다. 이 census를 완료하지 않으면
“사전 등록 contrast universe 전체로 확장”을 주장하지 않는다.

## P1 감사 결과와 Gate S0 작업 보드

| 항목 | Owner | 상태 | 산출물/근거 | Blocker 또는 next |
|---|---|---|---|---|
| E1–E6 research canon | Work | `USER-DIRECTED / CURRENT DESIGN` | `00_*.md`–`06_*.md`, through `DEC-P1-021` | execution remains blocked |
...
### FROZEN — E1–E6 current design canon

| ID | 항목 | 변경 권한 |
|---|---|---|
| F-01 | 연구 목적과 TUM2TWIN 사용 | 사용자 |
| F-02 | 여섯 experiment conditions와 각 역할 | 사용자 |
...
| F-12 | E3는 image-only/no-external-prior GS; E3–E6는 exact common base를 공유, E4/E5는 same ALS와 weighting만 다르고 E6는 LoD prior diagnostic | 사용자 |
| F-13 | Roofer/LoD2 O/X와 semantic textured mesh O/X를 독립 보고 | 사용자 |
| F-14 | changed + E3 X에서 prior reproduction은 허용하되 temporal currentness와 분리 | 사용자 |
...
| P-09 | P1 audit 뒤 P2 pre-result Gate S0에서 `EXHAUSTIVE_PARTITION` 우선, 불가능할 때만 `STRATIFIED_SAMPLE` | leakage·tuning 억제와 최종 coverage 명확화 | `U_target`/`E_pair(m)`/`E_all6` 수, E1–E6 비용, 공간 group, 목표 정밀도/검정력 | P1/P2 |
...
기존 4조건과 historical C1–C5의 충돌은 `DEC-P1-008`, 현재 E1–E6 전환은
`DEC-P1-021`로 해소했다. Numerical acceptance
...
- exact AOI와 `U_target`/`E_pair(m)`/`E_all6`
...
| Artifact | Status |
|---|---|
| `docs/research/00_*.md`–`06_*.md` | `E1E6_CANON_v3 / USER-DIRECTED DESIGN` |
...
| 여섯 조건이 같은 순서·명칭인가? | `E1`–`E6` canonical IDs 사용 |
| `L_upper ≠ P_LiDAR`인가? | 예 |
| `P_LoD1`이 roof topology를 주는가? | 아니오 |
| threshold/loss가 동결되었는가? | 아니오, `DEFERRED` |
| 현재 실행 정본은 무엇인가? | 00–06 E1–E6 design contract set; historical C/legacy plan은 계보 기록 |
| 지금 성능실험이 가능한가? | 아니오. `DEC-P1-021`은 문서·설계 amendment이며 별도 승인 전 새 실행 금지 |
...
| 2026-08-10 | `E1E6_CANON_v3` | 새 설계를 E1–E6로 전환하고 E4/E5 conflict contrast, dual-output O/X, prior reproduction temporal status를 동결; 과거 C-ID 기록은 보존 | 사용자 지시; `DEC-P1-021`; 새 실행은 별도 승인 전 금지 |
