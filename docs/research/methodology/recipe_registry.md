...
|---|---|---|
| `세션핸드오프_모집단잠금·기준문서_20260702.md` §1·§2 | 없음 | `docs/SESSION_HANDOFF.md`, `docs/population_verify.md`, `docs/experiments/research-operations/w_report_evidence/reports/W_report_evidence.md` |
| `P2_실험기록_20260629.md` | 없음 | `docs/experiments/joint-optimization/w_phase_b_structure/reports/W_phaseB_structure.md`, `docs/experiments/evaluation/w_observability/reports/W_observability_inventory.md`, `docs/W_observability_test.md`, `docs/experiments/evaluation/w_d12_metric/reports/W_D12_metric_final.md` |
| `원격발주_투영fix·LS정합·재게이트·재계산체인_레시피감사_20260702.md` Task B | 없음 | `docs/experiments/input-and-alignment/projection_datum/reports/projection_geoid_rootcause.md`, `docs/experiments/input-and-alignment/projection_datum/reports/projection_datum_fix.md`, `configs/input_and_alignment/projection_datum.json`, `phases/p2-gsjso/runs/20260702_*`, `phases/p2-gsjso/runs/20260703_*` |
...
|---|---|
| repo 규칙 | `CLAUDE.md` |
| D/D4/D5 사전등록 | `docs/research/preregistration/joint_optimization/P2_D4_사양서_사전등록_20260625.md`, `docs/research/preregistration/joint_optimization/P2_D5_cp_ablation_사양_사전등록_20260626.md` |
...
| C2 opacity 진단 | `C2(C-protect ckpt; alpha-gate bypass; no-train)` | 새 학습 레시피가 아니라 C ckpt의 Gaussian 위치를 opacity 무시로 추출. | `scripts/evidence_and_attributes/population_analysis/c2_dump_means.py`; `tum_mob_eval.py`; `tum_mob_ref_rms.py` | in-scope 0점 원인이 opacity인지 확인 | `docs/W_opacity_diag.md` | 진단 전용, 레시피 라벨 폐기 권장 |
...
| D4 corrected | `GS(D4; seed-protect; pho1·sem0.1·nc0.05·dep0.03·nrm-off·str1[g2;na0.08;cp0.01;warm15k]; gssem)` | `w_depth=0.03`, `w_normal=0`, `w_structure=1.0`, `w_structure_na=0.08`, `w_structure_cp=0.01`, `structure_warmup=15000`, `structure_grouping=g2`. | `configs/tum_mob/gs_d4_{dense,acmp,sparse}.yaml`; `run_d4.sh`; `src/stage2/train.py:main`; `tum_mob_tsdf_extract.py`; `_mob_prep_las_gssem.py` | D의 normal 과가중/깊이 noisy pinning 완화, cp 공정값. sparse config는 E5 A3 신규, ckpt 없음. | `results/tum_transfer/mob/gs_d4_{dense,acmp}/versions.txt`, `docs/experiments/joint-optimization/w_d4/reports/W_D4.md`, `docs/research/preregistration/joint_optimization/P2_D4_사양서_사전등록_20260625.md` | 후속 평가 기반. 정본 별명 미부여 |
| D5a | `GS(D5a; D4 except cp0; warm15k; gssem)` | D4와 동일, `w_structure_cp=0.0`. | `configs/tum_mob/gs_d5a_{dense,acmp}.yaml`; `run_d5.sh` | cp ablation off | `results/tum_transfer/mob/gs_d5a_{dense,acmp}/versions.txt`, `docs/experiments/joint-optimization/w_d5/reports/W_D5.md` | ablation 완료 |
| D5b | `GS(D5b; D4 except cp0.03; warm15k; gssem)` | D4와 동일, `w_structure_cp=0.03`. | `configs/tum_mob/gs_d5b_{dense,acmp}.yaml`; `run_d5.sh` | cp ablation hard | `results/tum_transfer/mob/gs_d5b_{dense,acmp}/versions.txt`, `docs/experiments/joint-optimization/w_d5/reports/W_D5.md` | ablation 완료 |
| D5c | `GS(D5c; D4 except cp0.01; warm5k; gssem)` | D4와 동일, `w_structure_cp=0.01`, `structure_warmup=5000`. | `configs/tum_mob/gs_d5c_{dense,acmp}.yaml`; `run_d5.sh` | cp ablation early | `results/tum_transfer/mob/gs_d5c_{dense,acmp}/versions.txt`, `docs/experiments/joint-optimization/w_d5/reports/W_D5.md` | ablation 완료 |
| B1 / D10 | `GS(B1; D4 + mvc0.5[nrm0.5;k2;angle40;baseline2;warm7k+ramp5k]; gssem)` | D4와 동일하고 `w_mvc=0.5`, `mvc_warmup=7000`, `mvc_ramp_steps=5000`, `mvc_neighbor_k=2`, `mvc_max_angle_deg=40`, `mvc_min_baseline=2`, `mvc_w_normal=0.5`. | `configs/tum_mob/gs_b1_{dense,acmp}.yaml`; `src/stage2/loss/multiview.py:l_multiview_consistency`; `src/stage2/train.py:_build_mvc_neighbors/main`; `run_b1.sh` | Phase B `L_mvc` 추가 실험 | `results/tum_transfer/mob/gs_b1_{dense,acmp}/versions.txt`, `docs/experiments/joint-optimization/w_phase_b_structure/reports/W_phaseB_structure.md`, `docs/W_observability_test.md` | Phase B 실험 완료. 판정 문구는 본 대장에서 채택하지 않음 |
| gssem read-out | `readout(gssem; semantic-TSDF[minobs3,voxel0.05]; Roofer eps0.3/minpts15/complexity0.888)` | 학습 레시피가 아니라 GS 의미 logits를 TSDF/점군 class로 변환한 read-out. D/D4/D5/B1 기록에서 기준 분류로 사용. | `scripts/stage3_readout/tum_mob_tsdf_extract.py`; `_mob_prep_las_gssem.py`; `tum_mob_eval.py --classifier gssem` | SMRF가 지붕을 ground로 먹는 문제를 분리/대체 | `docs/research/preregistration/joint_optimization/P2_D4_사양서_사전등록_20260625.md`, `docs/research/preregistration/joint_optimization/P2_D5_cp_ablation_사양_사전등록_20260626.md`, `docs/experiments/evaluation/w_gssem_requal/reports/W_gssem_requal.md` | read-out 이름. 학습 레시피 별명 아님 |
...
|---|---|---|
| `v5` | 현재 checkout에 `v5` config/run 이름은 없다. `docs/experiments/evaluation/w_generation_8way/reports/W_generation_8way.md`와 `analysis_pack_v6/versions.txt` 기준으로는 `v6`부터 추적 가능. | 매핑 불가 라벨. 숫자 인용 시 config/run명을 직접 써야 한다. |
...
| `D9` | `docs/experiments/evaluation/w_observability/reports/W_observability_inventory.md` 기준 `overseg-faithfulness`. D4/D5 output과 기존 점군을 분석. | 새 ckpt 없음. `face_support()` 분석 자산명으로만 유지. |
...
| `configs/input_and_alignment/tum_mob/seed_semantic.yaml` | `shift_z=604-45.7=558.3`, geoid path | semantic seed/label | E5 신규 canonical label 이력. |
...
| `scripts/input_and_alignment/preparation/tum_tsdf_extract.py`, `tum_qc_*` | `SHIFT=[690953,5336071,604]` | 3D extract/QC | image-projection config 미사용. |
