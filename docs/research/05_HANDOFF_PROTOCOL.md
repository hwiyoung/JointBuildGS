...
- `required_for_task: true`이면 handoff가 지정한 exact artifact records가 task
  ...
  최초 `artifact_verified` receipt(통상 Experiment Host의 `100-accepted`)는 push 전
  local receipt 검증과 push 후 exact `origin/main` 검증에서 live bytes를 각각 한 번
  전부 해시한다. 두 검증이 모두 PASS하면 immutable attestation이 확정된다. 후속
  `200`/`300` receipt는 그 attestation을 byte-for-byte 상속하고 Git receipt SHA와
  ancestry를 검증한다. 같은 immutable input을 이후 상태 전이마다 다시 해시하지 않는다.
...
- `required_for_task: false`이면 external payload를 cross-host 전달해야 task를
  ...
  조사 결과를 처음 `artifact_verified`로 승격하는 `200` receipt가 있다면 해당
  receipt의 push 전·후 검증에서 새 records만 각각 한 번 해시한다. 이후 successor는
  다시 해시하지 않는다. `300-closed`에서 처음 승격할 수는 없다.
