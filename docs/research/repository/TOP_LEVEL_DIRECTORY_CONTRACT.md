...
This contract defines why each top-level directory may exist and which information it owns. The control-plane migrations recorded under `docs/research/repository/migrations/` have now applied this contract to verified document, evidence, receipt, script, and test families.
...
The target control plane has seven permanent top-level directories.
...
| `scripts/` | Reproducible workflows need stable command-line entry points that connect configs to `src/`. | Training, evaluation, conversion, reporting, repository inventory, and human-inspection drivers grouped by purpose. | Reusable method implementations that should be imported from `src/`; hidden scientific constants; one-off shell history; generated outputs. | Regular Git; maintenance lives in `scripts/repository/`, inspection in `scripts/inspection/`, and scientific execution requires a run receipt. |
...
| Is it a maintenance or repository utility? | `scripts/repository/` |
| Is it a human inspection driver? | `scripts/inspection/`; browser application code belongs in `src/apps/` |
...
## Quarantine and runtime payloads

`external/`, `legacy/`, `data/`, `results/`, `reports/`, and `fair-pilot/` are not repository roots.
The 2026-07-30 migration preserved their bytes in sibling artifact storage and recorded the locations in
`artifacts/manifests/`. Docker mounts that backend once at `/artifacts/JointBuildGS`; current code resolves
payloads through `JBGS_ARTIFACT_ROOT`. A future approved third-party dependency must use a declared package,
submodule, or artifact contract rather than recreating an ambiguous top-level directory.
...
1. Verified reusable drivers are grouped by scientific purpose under `scripts/`; tests use matching purpose folders. Scientifically locked or phase-specific procedures remain phase-local.
...
4. `results/`, local `reports/`, and `fair-pilot/` were split by role and their former root paths were retired.
5. Historical and active non-index P2 payload is in the sibling artifact workspace with checked manifests; Git retains only compact phase control records.
