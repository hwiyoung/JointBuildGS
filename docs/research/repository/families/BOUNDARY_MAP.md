...
- `docs/evidence/archive/boundary_map/v4/tables/boundary_map_v4_ladder.csv`는 v4.1의 직접 입력이자 이전 스냅샷이다. 현재 배정에는 사용하지 않는다.
- `docs/evidence/archive/boundary_map/v4/tables/boundary_map_v4_targets.csv`는 v4 당시 64동 스냅샷이다. v4.1이 9행을 보강했으므로 현재 target/셀 값은 v4.1 ladder를 필터링해 읽는다.
...
개별 파일의 기계 판독 상태와 `supersedes`/`derived_from` edge는 `configs/repository/repo_inventory.json`의 `reviewed_family_maps`에 있으며, 생성된 `docs/research/repository/DOCUMENT_CATALOG.csv`와 `docs/research/repository/DOCUMENT_LINEAGE.csv`에 반영된다.

## 실행 영수증

| 버전 | 공개 결과 | 주요 실행 영수증 |
|---|---|---|
| v1 | `docs/evidence/archive/boundary_map/v1/` + v1 manifest | `phases/p2-gsjso/runs/boundary_and_robustness/20260716_boundary_map/` |
| v2 | `docs/evidence/archive/boundary_map/v2/` + supporting v2 files | `phases/p2-gsjso/runs/boundary_and_robustness/20260718_boundary_map_v2/` |
| v3 | `docs/evidence/archive/boundary_map/v3/` + supporting v3 files | `phases/p2-gsjso/runs/boundary_and_robustness/20260719_boundary_map_v3/` |
| v4 | current report/manifest/tables + `docs/evidence/archive/boundary_map/v4/` | `phases/p2-gsjso/runs/boundary_and_robustness/20260720_anchor_census/` |
