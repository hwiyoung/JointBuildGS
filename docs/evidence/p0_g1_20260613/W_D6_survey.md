...
> 출처: `d6_survey.py`. 산출(gitignore): `results/tum_transfer/mob/analysis_pack_d6/{survey_per_building,survey_by_type}.csv`. 그림 `docs/figs/W_D6_survey_overseg.png`.
...
## §3 그림
- [survey_overseg.png](figs/W_D6_survey_overseg.png) — (좌) ALS vs DIM target-only 면수 산점(84 both, 대각선=DIM=ALS; 4906969 강조): 다수 동이 대각선 위/주변에 분산, 둘 다 고-면수 도달 → 과분할은 광범위·LiDAR도; (우) 유형별 med(facets−ref): 평·경사 모두 ALS·DIM > ref.
