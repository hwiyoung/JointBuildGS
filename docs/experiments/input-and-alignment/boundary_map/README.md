...
활성 staged workflow가 고정한 v2 ladder도 현재 owner 경로 `docs/experiments/input-and-alignment/boundary_map/tables/boundary_map_v2_ladder.csv`로 해석한다. 이전 `docs/boundary_map_v2_ladder.csv` 문자열은 migration ledger에서만 보존하며, 새 코드와 문서는 이 디렉터리의 정본 경로를 사용한다.
