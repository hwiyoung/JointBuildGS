...
진행 상태(2026-08-12): **S0-a/S0-b/S0-c 완료** — 결과와 결정 입력은
`S0_RESULTS_ko_v1.md` 참조. 요지: 같은 계보에서는 레거시 프라이어도 Roofer O를
5→13으로 개선(net +8; 대시보드 격차는 전부 계보 효과), TIN 레이캐스트 투영 QA
3게이트 전부 통과(>2 m 70.5 %→10.7 %, 중앙값 0.105 m), 노멀 항은 Stage-3 판독의
하중 지지 부재로 확정(depth-only 금지).

### 8.1 단계-구현-결정 맵

S0는 새 학습이 아니라 **이미 봉인된 런들의 빠진 판정·계측**이다(레거시 E3/E4/E5
학습과 Roofer 산출은 techdev에서 완료되어 있었고, 레거시 E3의 v22 판정 칸만
비어 있었다). S1부터 새 학습이 들어간다.

| 단계 | 새 학습 | 선행 구현(§6) | 이 단계에서 동결되는 결정 | 산출 |
|---|---|---|---|---|
| S0 (완료) | 없음 | 없음 | — (D5·D7·D9의 실측 입력 확보) | 계보 분해, 투영 QA, 절제 기전 |
| S1 | 로컬 55뷰 소규모 | G6(정규화 opt-in), G3(투영기 v2 로컬), G2(conflict 채널) | **D2**(F1/F2/F1×F2), **D9**(빈 픽셀 A/B), **D7**(가중 재캘리브레이션) | 동결된 E4-v2/E5-v2 손실 구성 |
| S2 | 풀신 937뷰 ×2런 | G3 풀신 + §4.4 preflight, G8(로깅) | D1·D3·D5 적용 실행(결정 번호 필요) | E4-v2/E5-v2 체크포인트·Roofer 산출 |
| S2b | 풀신 ×2런 | G7(게이트드 시드 생성기) | **D4**(시드 정책 확정) | 2×2 매트릭스 완성 |
| S3 | 없음 | 없음 | — | 같은 계보 6조건 v22 표 + E2/E3 페어드 전이표 + 기술 Return |
| S4 | (별도 결정) | — | **D6**(E6 독립 소스) 외 | 합성-변형 강건성 diagnostic, E6-v2 |

S3 이후는 본 계획의 범위를 벗어난다: 방법 동결(P3)이 끝나야 별도의 확증 설계
(독립 테스트 설계, 임계 동결, 공식 G3/G4 — 현재 계약상 별도 결정 전 금지)로
넘어갈 수 있다.
