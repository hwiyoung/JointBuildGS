...
- **표준 보고 컷(2026-08-13 사용자 확정)**: 모든 판독은 **전체 93 / 채움-의존 /
  현재-지배** 3층으로 병기한다. 층 정의는 결과 변수가 아닌 **기준선 단독**
  (E2 completeness@0.25 < 0.9 등, 처치 arm 무관)으로 하고, 채움-의존층은
  **관측-구멍 vs 분류-구멍 분해를 병기**한다 — 4906982 실측(raw 기하 comp
  0.97 vs 봉인 class-6 계보 0.35)이 보여주듯 "MVS 붕괴"의 일부는 SMRF 분류
  실패이며, 이 경우 union 이득은 기하가 아니라 분류 경로로 흐른다(Claim A
  문장은 "관측+분류 공백의 충전"으로 조정 검토).
...
## 2.7 스코프 확장(2026-08-13 사용자 합의): 통합 제품 + 보정 씨앗

D1·E9 이후 사용자 합의로 저널1의 최종 산출을 "부품"에서 "통합 제품"으로 확장:

- **아키텍처 확정**: 기하 = 측정(MVS ± 판정 통과 prior 패치) / **판정 = 미분
  가능 렌더링**(수직-분리 패치의 지지·모순·판단불가 — E9 프런티어가 실험적
  필요성 근거) / 외관 = 학습 GS 베이킹. GS는 기하 생성자가 아니라 결합 규칙의
  판정 함수 + 외관 생성자.
- **E9C 통합 제품**: E9B 기하 + HV-3 판정 마스크 → Roofer LoD2 → GS 베이킹
  텍스처+시맨틱 mesh + 건물별 인증서(지지/모순/판단불가 지도). "prior를 사용한
  textured mesh"의 실물. 외관 정량 = held-out PSNR/SSIM.
- **bounded δ-보정 데모(박사 2장 씨앗)**: δ-주입 데이터(정답 기지)에서 렌더/
  에지 정합 최대화로 δ̂ 추정 → prior 재정렬 → E9 재실행 → 채움 품질 회수.
  "탐지→보정→회수" 루프의 최소 증명.
- 판정 학습화·다지역 일반화는 박사 후속 장으로 명시적 이월.
