...
따라서 현재 규칙으로 만든 428뷰 cache는 기제 상한을 깨끗하게 격리하지 못한다. 진단 스크립트는 전수 실행을 hard-block하고 `--view-stems/--limit` smoke만 허용한다. 산출은 `phases/p2-gsjso/runs/e5_c001/20260713_e5_c001_s3_track0/smoke_c00118_{docs,run}/`이며, 428뷰 생성·T0-4 프로브·Track A 학습은 시작하지 않았다.
...
- 산출: `docs/e5_c001_s3_seed_inventory.csv`, `phases/p2-gsjso/runs/e5_c001/20260713_e5_c001_s3_track0/t0_2_t0_3_manifest.json`.
...
| 단계 | 의미 개입 | file:line |
|---|---|---|
| Arm 1′ | `w_sem=0.1`, `sem_detach_geometry=false` | `configs/e5_c001/e5_s2p_interaction/gs_e5_C001_s2p_arm1p_dense_r1.yaml:48-50` |
| checkpoint→점군 | extractor에 `--no-sem` 없음 | `scripts/e5_c001/p2_gsjso/e5_c001_readout_ablation.py:226-267` |
| 의미 렌더·융합 | `sem_logits` alpha-composite→argmax→voxel majority | `scripts/e5_c001/p2_gsjso/e5_c001_readout_extract_ablation.py:208-223,270-324,341-378` |
| LAS 분류 | `P_class_clean` 필수, Roof/Wall→BUILDING, Terrain→GROUND, BG drop | `scripts/input_and_alignment/p2_gsjso/_mob_prep_las_gssem.py:71-77,108-136` |
| 발자국 | bbox+buffer crop·Roofer 입력·평가, class 배정자 아님 | `_mob_prep_las_gssem.py:79-89,151-164`; `e5_c001_readout_ablation.py:327-388` |
| 405 수리 | boundary ring reverse만, 점군·좌표·의미 재생성 없음 | `scripts/e5_c001/p2_gsjso/e5_c001_405_repair.py:110-128` |
...
- `phases/p2-gsjso/runs/e5_c001/20260713_e5_c001_s3_semantic_guided/{train_fingerprints.csv,readout_fingerprints.csv,downstream_versions.txt}`가 두 학습·점군화·하류 산출의 config/checkpoint/image/hash를 결박한다.
