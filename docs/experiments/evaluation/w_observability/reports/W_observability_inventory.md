...
## §6 장면 GS 커버리지 + 면 단위 자료 (인벤토리 후속, 읽기 전용·관찰만; 커밋 `facet-coverage-inventory`)

> ⚠ **§ 위 "상관 n 확대만 학습 필요"를 정정**: 장면 GS가 AOI 전체를 이미 덮어 **표본 확대 = 평가만(재학습 불요)**.

### §6.1 장면 GS 커버리지 [핵심] — **AOI 전체 덮음 (11동만 아님)**
- gs_d4_dense `final.pt` = **3.2M 가우시안**, UTM bbox **x[690094,691733] y[5334726,5336998]**(1.6×2.3 km) ⊃ AOI footprint 범위 x[690729,691230] y[5335833,5336374](0.5×0.5 km). → **장면 GS가 AOI 전체+여백을 덮음**(1회 장면 학습; 11동 한정 아님).
- AOI footprint **199동 중 GS≥1점 184동·GS≥50점 154동**. 양쪽-성공 71동·통제 93동(전부 AOI 내)은 DIM 성공 영역이라 GS 보유 → **71동 지붕에 가우시안 있음**(평가만으로 닿음).

### §6.2 표본 확대 비용 — **(a) 평가만(재학습 없음)**
- 인벤토리 미해소 "(a) footprint 클립+건물별 평가 vs (b) 건물별 새 학습" → **명확히 (a)**. 장면 GS가 이미 71동(최대 ~154동)을 덮으므로, 추가 건물 = `tum_mob_tsdf_extract.py`의 **`TARGETS` 목록 확장**(현 11동) → extract(기존 ckpt를 box로 클립) + gssem 분류 + Roofer. **재학습 0.**
- 비용: 현 11동 extract+eval ~5분; ~70동이면 박스 union↑로 ~20–30분 추정(학습 아님). → **11→40~70(최대 ~154)까지 평가만으로 확대 가능.** 단 baselines.json·`TARGETS`에 동 추가 필요(코드 1줄 수준).

### §6.3 D6 census (통제 11동 지붕면 수)
| | ref | als | dim | gs_dense(mob harness) |
|---|---:|---:|---:|---:|
| 11동 합계 | **17** | **23** | 15† | 25† |

동별(ref/als): 42364609 1/1·42364659 2/4·42364663 1/1·4907182 2/3·4907510 1/3·4908050 1/1·4908166 1/1·4908176 1/1·4906969 3/4·4908023 1/1·4906972 3/3. †dim·gs_dense 컬럼은 census harness서 **DIM 실패 8동 공란**(과거 부분기록·stale 주의) → **GS 면수 정본 = `complexity_metric.csv` `n_faces`**(아래 §6.5).

### §6.4 D9 면별 resid·받침(k/m) — **디스크 존재(5동), 면-ID 연결=GS면↔ALS점**
- `results/tum_transfer/mob/overseg_lever/b1_faithfulness_faces.csv`(59 면-행, 양 arm). 컬럼 `bid·arm·face·face_z_ortho·n_als·resid_med·resid_abs·supported`. + `faithfulness_faces_<bid>.csv`(4동).
- **현 면별 resid = 5동만**(overseg_b1_faithfulness BLD 한정: 4906969·42364659·4907510·4906972·42364663). gs_b1_dense 면별: 4906969(10면 k1/m9 resid 0.34–5.22)·42364659(4 k0/m4 0.65–1.75)·4907510(4 k1/m3 0.22–3.47)·4906972(10 k7/m3 0.10–1.91)·42364663(1 k1/m0).
- **연결 방식**: `face_support`는 **GS 면 폴리곤 → 그 안의 ALS 점**(XY 포함)으로 매칭해 면별 n_als·resid·k/m 산출. **GS면↔ALS면/ref면 (면-대-면) ID 연결은 없음**(ALS=점, ref=별도 GML 면). 면 키 = (bid, arm, face 인덱스).

### §6.5 면별 텍스처 가능성 + 건물 내 변동
- **GS 면 총수(면 단위 표본 상한)**: b1 **35면**(9동)·d4 36면(10동). ⚠ **집중**: 4906969(10)+4906972(10)=35 중 20면; 나머지 7동은 **1–3면**(단순 박공·평).
- **면별 텍스처 = 가능(미산출)**: 입력 전부 닿음 — 면 폴리곤 `parse_solid_roof(arm,bid)`(11동 Roofer cityjsonl)·near-nadir 영상 962 JPG·COLMAP 포즈 `sparse/0`·09/11 투영 로직 재사용. footprint 대신 **각 면 폴리곤을 near-nadir 영상에 투영**하면 면별 텍스처 산출 가능(읽기 전용·학습 아님·신규 스크립트 필요, 본 인벤토리선 미실행).
- **건물 내 변동(디스크 자료=결함측 프록시)**: 면별 **resid 범위**(건물 내) — 4906969 **4.88**·4907510 3.25·4906972 1.81·42364659 1.10·42364663 0. → **다면 건물서 면이 실제로 갈림**(일부 받침·일부 떠있음). 텍스처측 변동은 미산출이나, **건물 내 대비 검정이 의미있는 동 = 면≥4인 4동(4906969·4906972·42364659·4907510)뿐**; 7동은 1–3면이라 건물 내 변동 약/무.

### §6.6 요약 세 줄 (판정 금지)
1. **면 단위 (텍스처×결함) 짝 — 지금?**: 결함측(면별 resid) **5동 ~29면 디스크 존재**(전 11동 확대 = complexity_metric BLD 11동 재실행, 평가만·분). 텍스처측은 **면별 텍스처 1회 산출(09/11 면-적용, 읽기 전용) 선행 필요**. → 짝 만들려면 면-텍스처 1회 런; **표본 ≈ 35면**(단 4906969+4906972가 20면 = 편중).
2. **건물 표본 — 평가만으로 몇 동?**: 장면 GS가 AOI ~154동 덮음 → **11 → 71(양쪽성공) 또는 최대 ~154까지 평가만(재학습 0)**, `TARGETS` 확장 + extract/classify/Roofer ~20–30분(70동).
3. **건물 내 텍스처 대비 — 몇 동?**: 면≥4 = **4동만**(4906969·4906972·42364659·4907510) 건물 내 대비 의미; 결함측 건물 내 변동 확인됨(4906969 resid 범위 4.88). 나머지 7동 1–3면이라 약/무. → **건물 내 대비 검정은 4동 표본**(작음).

> 재현: 읽기 전용. GS 커버리지 bbox = gs_d4_dense `final.pt` means+SHIFT[690953,5336071,604]. 면별 자료 = `b1_faithfulness_faces.csv`·`complexity_metric.csv`. 면별 텍스처 입력(폴리곤·포즈·영상·09/11) 가용·미실행. 새 학습·재구성·data/raw 변경 없음.
