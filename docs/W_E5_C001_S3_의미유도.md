...
| 코드 | 쉬운 이름 | 잠금 예측 | 관찰 |
|---|---|---|---|
| P-I | 신호 도달 | 1k 게이트에서 무늬없음 3채 픽셀에 `L_semdepth` 기울기가 0이 아님 | attempt 1: 4907199·8568392 양수, 8568391 0; half-once 재게이트 대기 |
...
| 1k 게이트 실행·CSV | `docs/e5_c001_s3_loss_gate_audit.csv` | **attempt 1 완료·CSV 417행: semdepth 절반 1회 재게이트 분기** |
...
attempt 1 게이트 상태는 `fail`이며 사유는 `L_semdepth` 상한 초과 + 8568391 P-I 0이다. 사전 규칙대로 `L_smooth/L_plane` 둘만 0.25/0.25→0.125/0.125로 비례 절반한 attempt 2를 한 번 생성한다. `L_nb=0.01`은 유지한다. full 학습은 attempt 2 결과·CSV 커밋 전까지 보류한다.
...
| gate attempt 2 | 2001 | 2,500 | 0.125 smooth + 0.125 plane + 0.01 nb @1.5k | **half-once config 생성·미실행** |
