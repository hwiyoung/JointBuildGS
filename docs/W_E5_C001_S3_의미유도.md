...
| 코드 | 쉬운 이름 | 잠금 예측 | 관찰 |
|---|---|---|---|
| P-I | 신호 도달 | 1k 게이트에서 무늬없음 3채 픽셀에 `L_semdepth` 기울기가 0이 아님 | **미달**: 두 attempt 모두 4907199·8568392 양수, 8568391 0 |
...
| 1k 게이트 실행·CSV | `docs/e5_c001_s3_loss_gate_audit.csv` | **attempt 1+half-once 834행 완료: 최종 gate fail, Track A 보류** |
...
| P-I 4907199 | 양수 region 8/25, max share 0.36157, nonzero px max 1,575 | pass |
| P-I 8568391 | 양수 region 0/8, max share 0, nonzero px max 0 | fail |
| P-I 8568392 | 양수 region 1/14, max share 0.99987, nonzero px max 12 | pass |

attempt 1 게이트 상태는 `fail`이며 사유는 `L_semdepth` 상한 초과 + 8568391 P-I 0이다. 사전 규칙대로 `L_smooth/L_plane` 둘만 0.25/0.25→0.125/0.125로 비례 절반한 attempt 2를 한 번 실행했고, `L_nb=0.01`은 유지했다.
...
| P-I 4907199 | 양수 region 7/25, max share 0.98110, nonzero px max 6,595 | pass |
| P-I 8568391 | 양수 region 0/8, max share 0, nonzero px max 0 | **fail** |
| P-I 8568392 | 양수 region 2/14, max share 0.97333, nonzero px max 189 | pass |

attempt 2는 비중 상한은 모두 통과했지만 8568391 P-I 0이 재현돼 최종 `gate_status=fail`이다. 이후 가중 조절은 허용되지 않는다. 분기표대로 `L_semdepth`는 Track A 투입 불가로 기록하고, `L_nb`는 자체 게이트 수치를 통과했지만 단독 full cell은 발주 범위에 없으므로 시작하지 않았다. S3-A r1/r2는 보류·재소집 상태다.
...
| gate attempt 2 | 2001 | 2,500 | 0.125 smooth + 0.125 plane + 0.01 nb @1.5k | **종료: 비중 통과, 8568391 P-I 0 재현 → gate fail** |
| S3-A r1 | 2001 | 30,000 | gate 선택 가중 이관 | **게이트 분기로 미실행** |
| S3-A r2 | 2001 | 30,000 | gate 선택 가중 이관 | **게이트 분기로 미실행** |

## 3. 5축 채점표와 합격선

| # | 축 | S3-A 합격선 | r1 | r2 |
|---|---|---|---|---|
| 1 | 완전성 | **(i) 무늬없음 3채 중 `>=1` 정품 조립** = 참조 RMS `<=3m` AND val3dity 유효 AND 완전성 `>=0.5` | 미실행 | 미실행 |
| 2 | 정확도 | **(ii) 양쪽 성공 6동 Arm 1′ 대비 짝 delta 중앙 `<=+0.3m`, `>+1.5m` 파국 0** | 미실행 | 미실행 |
| 3 | 형상 | 합격선 없음. 경사 모드·방위각; “RMS 좋고 모드 0” 성공 불인정 | 미실행 | 미실행 |
| 4 | 유효성 | 합격선 없음. 유효 조립 수·302 추이 | 미실행 | 미실행 |
| 5 | 청소 | 합격선 없음. `N<=1,150,636`, `rend_dist<=0.5` 유지 관찰 | 미실행 | 미실행 |
...
|---|---|---|---|
| 무늬없음 3채 정품 조립 | 0 | 미실행 | 미실행 |
| 양쪽 성공 6동 짝 delta | 0(대조축) | 미실행 | 미실행 |
| 4907185/60098 경사 모드 | base CSV 재사용 | 미실행 | 미실행 |
| 4907202 5k→10k 생존 | `463→38`(8%) | 미실행 | 미실행 |
...
- 최종 게이트 증거 = `docs/e5_c001_s3_loss_gate_audit.csv` 834행 + attempt별 cache-preflight/version 파일. attempt-1 P-J/P-L lock SHA-256은 `bbf6611a5eeb5e59796c8acb9131e44076323f566db7e38ff60399aef636b074`다.
- Track A 분기가 열리지 않아 `arm_cells`, 405 재채점, timeline, densify, gable, rend-dist, z-hist, sheet, 8way/5축 패널은 생성하지 않았다. 이는 산출 누락이 아니라 잠긴 gate-fail 분기의 결과다.
- `docs/issues.md`에 주소 재규정·T0-1 fail-fast·provenance·gate attempt 1/2를 기록했다. 정확한 최종 repo 커밋 해시는 회신 본문에 명기한다.
