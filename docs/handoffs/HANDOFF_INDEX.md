...
| `P2-W2C-GATE-S0-REMEDIATION-R1-v1` | P2/S0 | Work→Codex | v1 | `APPROVED_FOR_EXECUTION / OFFER PENDING` | `0928201553ba414109ae1f547a8e18a0be38b3d4` | `docs/handoffs/returns/P2_C2W_GATE_S0_REMEDIATION_R1_RETURN_v1.md` | — | `GRANTED — 2026-07-31` |
...
P1 v1 technical chain은 `offered → blocked`로 종료됐으며 재개하지 않는다. P1 R2와
Gate S0 preparation v1은 각각 `offered → accepted → verified → closed`로 종료됐고
writer 순번이 Work Host에 반환됐다. Gate S0 preparation의 기술 완료는 Gate 승인과
다르다. `BLOCKED_FOR_GATE_S0_REVIEW`는 Experiment Host의 제안 상태이고 human Gate
decision은 pending이다. 다음 실행에는 새 remediation packet과 새 handoff ID가 필요하다.
