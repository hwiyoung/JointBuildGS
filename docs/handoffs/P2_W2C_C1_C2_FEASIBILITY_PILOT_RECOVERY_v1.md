# Work-to-Codex Task Packet — P2 C1/C2 feasibility pilot recovery v1
...
Repeat no scientific work from the closed failed namespace. In a new add-once
namespace, repair only these two pre-scientific execution defects recorded by the
closed Return at commit `e596438e12041118a93fbf71e0fc29ad73b0fa81` and closed receipt
at commit `896fe284bc4d496e6e9c79720f4e75396a41d0b2`:

1. validate the inherited artifact-verified accepted receipt without mounting the
   artifact root or rehashing the five already attested compact inputs;
2. make the machine decision channel unambiguous even when the immutable project
   image emits CUDA/NGC startup text.

After a new zero-scientific synthetic smoke passes, run the already approved exact
51-building development-only C1/C2 feasibility pilot. No scientific input,
condition, parameter, roster, split, reference, metric contract or cost cap changes.

## Frozen scientific contract

- Common current source remains exact `962 / 937 / 25`.
- `U_target=199`; evaluation candidate set remains 72 in nine groups; this task opens
  only the exact 51 development buildings in five groups.
...
- C1 and C2 keep the same `R_derived`, adapter, pinned Roofer image, parameters and
  validators. No external roofprint is allowed.
- Canonical G2 remains null with `CANONICAL_VALIDATOR_UNAVAILABLE`. G3, G4 and
  `PASS_usable` remain null with `THRESHOLD_NOT_FROZEN`.
- LoD2-derived LoD1 is not accessed. It remains diagnostic/self-conditioned and
  primary-C5-ineligible under `DEC-P1-011`.
- C3–C5, Fusion W1, `R_ext`, validation and held-out performance remain prohibited.
- `scientific_verdict` remains `null`.

The exact compact inputs, hashes, development roster, Stage-3 controls, score-only
reference order, retry rules, 12-hour wall-clock cap and 100 GB output cap remain as
specified by the activated original packet at source commit
`d5265d9afbe9afcd49e2bedd5900c3026f7a3b2f`. This recovery packet does not authorize
the failed packet or namespace to be reopened; it carries the unchanged frozen values
forward into a new operational event.

## New add-once identities and write scope
...
4. A regression test injects CUDA/NGC-like banner lines and proves the exact machine
   action is recovered once.
5. A regression test proves the wrapper contains no post-acceptance artifact-root
   validation/full-input hash path and uses the new namespace only.
6. Network-disabled Docker tests, zero-scientific preflight, shell syntax validation
   and instruction-sync tests pass at the exact source commit.
7. Independent reviews cover scientific scope/leakage, no-repeat/reproducibility and
   two-host ownership/path isolation before activation and again before `200`.

## Experiment Host lifecycle after activation
...
On a stop condition, do not salvage or rerun the namespace. Return honest evidence,
close the receipt chain, return writer ownership and keep `scientific_verdict: null`.

## Done when

- the new smoke passes without scientific access or repeated hash;
- all exact 51×2 development rows are present without exclusions or hidden failures;
- C1 self-reference and C2 independent-reference results remain separated;
