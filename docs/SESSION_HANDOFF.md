...
> ⚡×15 **D12 observability-test — 관측가능성↔결함 + B1 생사 (2026-06-30, `feat/p2-structure-learn`, 커밋 `observability-test-d12`, 미푸시)** — **재학습 없음**(평가 확대+읽기분석), 관찰만, 판정=김휘영. 본보고 `docs/W_observability_test.md`(+G1 사본). 재현 `run_d12_eval{,_chunked}.sh`·`d12_defect.py`·`d12_analysis.py`·`d12_facet_figs.py`.
> **Step0 평가확대(재학습0)**: gs_d4·gs_b1 ckpt 클립→gssem→Roofer 3축결함 **11→79동**(8무텍스처실패+71생존). ⚠71-box extract OOM(rc137 surf 318M점)→**10동 청크 8개**로 해소. raw_lidar=ALS smrf 79. 결과 gs_d4 78·gs_b1 77동(d12_defect.csv 155행·2270면). 3축=높이(dz강건 resid·k/m)+기울기(법선각)+수평(면매칭 근사)+PSD. **결과(결정적·부정 우세)**: **A 관측가능성↔결함=약-무상관**(텍스처 r≈−0.06 전축 p>0.3·n78; 커버리지 약·**역방향**=복잡도 교란) + **⭐무텍스처-실패 8동이 오히려 저결함**(높이중앙값 **0.5 < 생존 1.43**=가설 정반대; 무텍스처지만 기하단순). → **"결함=관측가능성" 재정의 미확정/반증**, D11(결함≠복잡도)와 합쳐 **결함=per-building 특이적·관측가능성 무관**. **C B1 생사=죽임**(사전등록 2기준 둘 실패: 받침 Δ+0.039 Wilcoxon p=0.36·McNemar 잃197/얻222 p=0.24=FAIL; 텍스처편상관 r=−0.04 p0.77=FAIL) → **다음=깊이·법선 prior(다시점 종료)**. **D 생성 보편**(텍스처/커버리지 70동 중 70 GS성공·무텍스처8동도 복원=클래스불균형 로지스틱불가)→관측가능성은 생성 안 가름. 오라클상한 79(T7)/93(census). **B 면단위**: 건물내 결함변동 큼(108580336 112면 resid범위 27m 등 facet_*.png), 단 **면별 텍스처 미산출(보류·09/11 투영 가능)**. **권고(판정=김휘영)**: B1 죽음→**깊이·법선 직접감독**; 깨진축이 관측가능성과 안 묶이니 **결함=재구성품질 문제로 재프레임**; 면별텍스처 1회 산출로 §B 마무리·slope/수평 메트릭 정련. ⚠메트릭 caveat: slope 슬리버/희박ALS 노이즈·텍스처 표본 생존동 범위제한. 상세=[[project_p2_phaseB_b1]].

> ⚡⚡⚡⚡⚡⚡⚡⚡⚡⚡⚡⚡⚡⚡ **facet-coverage-inventory — 장면 GS 커버리지 + 면 단위 자료 (2026-06-30, `feat/p2-structure-learn`, 커밋 `facet-coverage-inventory`, 미푸시)** — **읽기 전용**, 관찰만, 판정=김휘영. `docs/experiments/w_observability/reports/W_observability_inventory.md` **§6 추가**.
> **핵심 정정**: 장면 GS(gs_d4_dense `final.pt` 3.2M 가우시안, UTM bbox x[690094,691733] y[5334726,5336998])가 **AOI 전체 덮음** — AOI 199 footprint 중 **GS≥50점 154동**(양쪽성공 71동·통제 93동 포함). → **이전 인벤토리 "표본 확대=학습 필요"를 정정: 표본 확대 = 평가만(재학습 0)** — `tum_mob_tsdf_extract.py` `TARGETS`(현 11) 확장 + extract(ckpt 클립)+gssem+Roofer ~20–30분이면 **11→71(최대 ~154)동**. **면 단위**: GS 면 총 b1 35면(9동)·d4 36면 — ⚠편중(4906969 10+4906972 10=20면, 나머지 7동 1–3면). 면별 resid·k/m **5동(~29면) 디스크 존재**(`b1_faithfulness_faces.csv`; GS면↔ALS점 폴리곤매칭, 면-대-면 ID 없음), 전 11동은 BLD 11동 재실행(평가만). 면별 텍스처=**가능·미산출**(폴리곤 parse_solid_roof·near-nadir 962JPG·포즈·09/11 닿음). **건물 내 대비 검정 의미동=면≥4인 4동만**(4906969·4906972·42364659·4907510). **요약**: 면단위 짝=면-텍스처 1회 산출 선행(표본 ~35면, 편중); 건물표본=평가만 11→71~154; 건물내대비=4동(작음). 상세=[[project_p2_phaseB_b1]].

> ⚡⚡⚡⚡⚡⚡⚡⚡⚡⚡⚡⚡⚡ **observability-inventory — 관측가능성 자산 인벤토리 (2026-06-30, `feat/p2-structure-learn`, 커밋 `observability-inventory`, 미푸시)** — **읽기 전용**(재학습·재구성·data/raw 무변경), 관찰만, 판정=김휘영. 본보고 `docs/experiments/w_observability/reports/W_observability_inventory.md`(A~E 표). 새 스크립트 없음(기존 P0/D9~D11 산출 확인만).
> **목적**: D11이 "결함=복잡도 아닌 영상 관측가능성" 시사 → 다음 실험 1순위(관측가능성↔결함 상관)·2순위(B1 메커니즘 점검) 자산이 repo에 있나 확인. **결과**: **A 텍스처·커버리지 있음(분할)** — P0 T7(`W3_failure_diagnosis_building_metrics.csv` 79동: view_count·incidence·occlusion_risk·dim_density·hole_ratio = **11동 공통 커버리지축**)·T9(`W3_failure_surface_cause_*` 8동: near_nadir_texture_gradient·shadow)·T10/T11(survivor 71: sharp_low_texture_pixel_ratio·texture_deficit). ⚠텍스처는 T9(8)·T11(3) 컬럼 상이 → 11동 단일 텍스처는 09/11 1회 재실행 필요(입력 디스크 존재: 영상 1899JPG `data/work/images/Images`·포즈 `data/work/colmap/sparse/0`·footprint gpkg; 학습 아님·~분). **B 결함 정본 있음** = `complexity_metric.csv` `bestfit_resid`(d4 10동·b1 9동). **C ⚠무텍스처 실패 8동 = GS 11동의 부분집합**(GS-JSO 회복)이라 n 11→확대 안 됨; 키우려면 추가 GS-JSO 또는 DIM-결함 프록시(T10 n=71). **D B1 점검 즉시 가능** = D10 건물별 Roofer Solid+las(11동, gssem)·D9 `face_support`(overseg_faithfulness.py) 디스크 존재→재학습 없이 k/m·면별 resid 재분석. **E 비용** = 장면 전체 ~5.2h/arm(건물별 학습 없음); w_mvc 스윕 ~5h/값·2-GPU 2값 동시. **종합**: **2순위 즉시 가능(선행無)**; **1순위 커버리지축 즉시·텍스처축만 09/11 1회 재산출 선행 권장**; 둘 다 추가 학습 없이 착수(n 확대만 학습). 상세=[[project_p2_phaseB_b1]].

> ⚡⚡⚡⚡⚡⚡⚡⚡⚡⚡⚡⚡ **complexity-survey — 메트릭 고정 + 복잡도 매핑 (2026-06-30, `feat/p2-structure-learn`, 커밋 `complexity-survey`, 미푸시)** — **재학습 없음**(기존 GS11·ALS·LoD2·Roofer 재사용), 관찰만, 판정=김휘영. 본보고 `docs/experiments/w_complexity_survey/reports/W_complexity_survey.md`. 재현 `complexity_metric.py`(Part1)·`complexity_survey.py`(Part2~4).
> **무엇**: B1 confound ②(메트릭 결함)·③(표적 단일레벨) 제거(①=w_mvc 스윕은 다음). **Part1 메트릭 고정(최우선)**: per-arm dz+`max(0,)`clamp → **clamp-free dz-강건**(best-fit=곡선최소 + joint-shared-dz). **신뢰본이 `overseg_b1_faithfulness`(per-arm dz) 대체.** **B1 재평가 정정**: 42364659 "4.8×"=**clamp 거품**(공정 best-fit 2.65→0.95≈2.8×, **B1 실개선 잔존**)·4907510 유지(2.56→1.74)·**정본 4906969 1차 "판정불가"→dz-강건서 B1 소폭우위**(1.60→1.52). **B1 핵심거동=평균회귀**(corr(d4_resid,B1−D4)=**−0.82**: 고-결함 개선·저-결함 소폭악화) → 1차 "거의 인공산물" **부분 정정**(고-결함서 실재). **Part2 복잡도(93)**: roofType 1000/3100/3200=40/34/10. ⚠ Roofer 면 mean-z가 박공 경사면을 "레벨"로 과셈("raw 다층 48" 거품) → **진짜 STEPPED-FLAT(1000+≥2레벨+span≥3m)=18동·GS 보유 3동뿐**(4906969·42364659·42364663) → **단차평 15동 GS 미보유**=confound③ 확인. **Part3 결함↔복잡도(n=9)**: **약-무상관**(span 0.23·레벨 0.05·평지붕 0.34만 약신호) — 최고결함에 단일레벨 4907510 포함, 최고span 박공 4906972 저결함 → **"결함=다층" 부분만 사실**, 결함은 복잡도보다 per-building 품질 좌우. **Part4 표적**: GS보유 다층=42364659·4906969·42364663(+고결함 단일 4907510); GS-needed 단차평 상위 4906982(5lv·33m)·60042(fpV187)·4906975(7lv)·4959336·108580335; 복잡박공 4906965(35m·54면)·108580336(9lv모임). **권고(판정=김휘영)**: dz-강건 메트릭 채택·**w_mvc 스윕을 고-d4_resid 동(42364659·4907510·4906969)**에(다층 라벨만으로 표적X)·단차평 표본 부족시 GS-needed 단차평 2~3동 GS-JSO·depth_mae/plane_rms 가드. 상세=[[project_p2_phaseB_b1]].

> ⚡⚡⚡⚡⚡⚡⚡⚡⚡⚡⚡ **Phase B / B1 — 자기지도 다시점 평면 일관성(L_mvc) 학습 완료 (2026-06-30, `feat/p2-structure-learn`, 커밋 `phaseB-b1-mvconsist`, 미푸시)** — **재학습 있음**(이 항만 격리), 관찰만, 판정=김휘영. 본보고 `docs/experiments/w_phase_b_structure/reports/W_phaseB_structure.md`(§0 Step0 + §2 구현 + §0″/§3~5 정정본). 재현 `run_b1.sh`·`overseg_b1_faithfulness.py`(--arms·shared-dz).
> **무엇**: cp(평면성)의 약한 고리(손정 0.5m 묶기, ±1~2.5m 못 끌음)를 영상 다시점 증거로 보완. **legacy `planarsplat_ref/loss_util.py:276` ULSR-GS 손실 이식**(w2c-네이티브) → `src/stage2/loss/multiview.py` + 포즈기반 이웃색인(train.py `_build_mvc_neighbors`) + gs_d4 복사+키1개(`gs_b1_{dense,acmp}`, cp=0.01 유지·격리). 2-GPU 5h13m, MVC 30k까지 건강(n_inlier ~1M).
> **결과(검증 정정본·적대 3렌즈)**: **B1은 설정대로는 "떠있는 면→실 층 높이" 목표 명확 미달성**. ⚠ **1차 헤드라인(복합 4.8×개선·val3dity F→T·PSNR−0.3) 대거 정정**: ① 42364659 "4.8×"=**per-arm dz 재중심화 인공산물**(공유-dz서 0.95→3.04 소실, strict 받침 0/4); ② PSNR −0.3=오류 → **held-out eval/psnr +0.11(무회귀)**, 단 **eval/depth_mae +3~5% 회귀**(숨은 비용); ③ val3dity F→T=**acmp arm만**(dense는 42364659 T→F 악화); ④ "복합/단차" 4동 실은 **단일레벨**(끌 단차 거의 없음). **공유-dz 견고 결과 3건뿐**: 4907510 개선(유일 견고)·42364663 개선·**4906972 박공 악화**(과분할+새 떠있는 면); **정본 4906969=dz-민감 판정불가**(plane_rms로는 악화 1.71→2.02). 전체 AOI **arm-의존**: dense 회귀(plane_rms +27%·생성 −1), acmp ~중립(val3dity 5→7). **lever 약함**(loss_mvc≈total 2%). **진단**: 레버 약·메트릭 결함(per-arm dz+clamp must-fix)·표적 부적합(단일레벨)·읽기 ~528 절단. **권고(판정=김휘영)**: 곧장 B2 비권고 → ①메트릭 고정(공유-dz·clamp제거) ②w_mvc 상향(0.5→1~3)+depth_mae 가드 ③진짜 다층 단차 동 표적 ④dense 회귀 진단. **⚠ 하네스 함정 재확인**: run_b1 gssem→smrf 순차라 **smrf가 B1 디스크 cityjsonl 덮음** → 첫 비교 B1-smrf vs D4-gssem 오류, **gssem 재평가로 디스크 복원**(smrf 백업 `gssem_requal_backup/perbuilding_b1_smrf.tar`) 후 재비교. 코드/구현은 검증 통과(스모크 깊이 40만 인라이어·수렴모델 법선 73.9만 corr); 문제는 실험 결과 해석.

> ⚡⚡⚡⚡⚡⚡⚡⚡⚡⚡ **overseg-faithfulness-diag (실험 2: 위치정확도+cp 메커니즘, 재학습 없음) 완료 (2026-06-29, `feat/p2-fidelity`, 커밋 `60a4771`, 미푸시)** — 관찰만, 판정=김휘영. 본보고 `docs/W_overseg_faithfulness.md`(+G1_package, 그림 6: 면받침4+UAV2). 재현 `overseg_faithfulness.py`·`overseg_uav_crop.py`.
> 질문(Phase A 미해결): GS 과분할 면들이 **결함**인가 **ALS가 못 잡은 실구조(충실)**인가? (ref·ALS 둘 다 라이다 단순화). **답(검증 정정본, 적대 3렌즈)**: **결함(defect)** — 반대가설(GS가 ALS 놓친 실구조 포착) **검증 실패**(4906969 떠있는면 8/9 아래 raw ALS 0~6%만 0.5m내·이차클러스터 없음; ALS는 실 3레벨에 앉고 GS만 벗어남). 정량(정본 4906969, ALS 3336점): 최선 정합서도 **14면 중 5 받침/9 어긋남**(=허위1[510.8 −12.6m]+한레벨내2+**±1~2.5m 어긋남6**); **대조 박공 k=3/3가 방법 검증**. 즉 **결함 아형=±1~2.5m 높이오차(통째 날조 아님)**, **복합 지붕**(UAV=반원 데크+중앙 HVAC 단) 위. GS는 ALS보다 정확하지 않음. **cp 정의**(코드): `L_coplanar=mean(n_k·c_i+d_k)²`(그룹평면에 중심 끌기, n_k/d_k detach); **g2 grouping=voxel2m+26이웃+|n·n|>0.92 AND point-to-plane<merge_d_tol 0.5m, 단일연결** → **단차-인지**(>0.5m 단차 병합 차단, Phase-A facet-merge 무제한 chaining과 다름)이나 **0.5m-tol이 ±1~2.5m within-level wavy는 못 끌어옴**=한계. **⚠ D5 1차 "cp-early 단차붕괴·530상실" = 검증으로 반전(must-fix)**: 어느 arm도 530 미도달(전부 ~528=공통 readout 절단), d5c span 17.3→5.8 축소는 **허위 저면 제거**(실 단차 보존)·d5c가 떠있는면 최소(가장 깨끗) → "cp 반드시 late" 처방 **철회**(early-fire 붕괴는 코드상 이론적, D5 미실현). **Phase B 입력(판정=김휘영)**: 결함이므로 완화 옳으나 **목표=면수 아닌 높이 교정(±1~2.5m를 실레벨로)**; "면수↓ 접고 ④정확도 전환"은 **비지시**(충실 아님); **structure-G2가 어긋난 면을 실레벨로 끌되 단차(>0.5m) 보존(적응 tol)** 지시, 단순 cp강화(D5b 과분절)·facet-merge(단차붕괴) 비지시. 검증 must-fix 2·should-fix 6 반영.

> ⚡⚡⚡⚡⚡⚡⚡⚡⚡ **overseg-lever Phase A (과분할 진단 + 후처리 레버, 재학습 없음) 완료 (2026-06-29, `feat/p2-fidelity`, 커밋 `6202b8b`, 미푸시)** — 관찰만, 판정=김휘영. 본보고 `docs/W_oversegmentation_lever.md`(+G1_package, 그림 7). 재현 `run_overseg_phaseA.sh`·`overseg_{smooth,analysis}.py`.
> 질문(실험1 후속): GS 지붕 과분할(4906969 14 vs ALS 5 등) 원인(밀도/거칢/상호작용)? 후처리 레버로 ALS 면수 수렴(표면 보존)? **⚠ 1차 헤드라인이 적대 3렌즈 재검(numerical/cause/lever-fairness)으로 정정됨(§0′·must-fix 3)**: ① **facet-merge는 ALS 면수에 2/6 도달**(1차 "0/6"은 보수적 임계 산물; a25/o1.0 가드안전서 4906969 14→**5**·4907510 6→**4**) — **단 실 단차 붕괴로 도달**(z-span **1.77·3.38m**, fitRMS 2~3.5×↑); ② **원인=다층(단차) 지붕 복합도**(저주파 "waviness" 아님 — LF metric ALS≥GS·1점의존; 4906969·42364659 동향면-다높이 **동일 메커니즘**); ③ **밀도=변조하나 ALS 미도달**(ceil 5/20/200→7/14/17·decim 14→17). **답(GS vs ALS, 정정본)**: Step0 Roofer v1.0.0 기본값·결정론적(3×=14/14/14)·ceil20. **레버**: smooth **죽음**(미감소·strong도 14→10>5 + **가드 파괴** 4906972 3→7)·decim **실패**(14→17)·**facet-merge만 가드안전**(2/6 ALS면수 도달이나 실구조 파괴+표면충실 미개선[점불변·3/6동 roof-env RMS 1.7~2.6×]). 판정표(§6): **4906969·4907510=부분**(면수도달·실단차파괴), 42364659·4908023=무효, 가드(4906972·42364663) 유지. **한 줄: "후처리는 CityGML 면-절약(parsimony)을 살 뿐 표면충실·실 단차구조 보존은 못 산다" → Phase A 부족 → Phase B 권고**(cp 재학습=D5 부분레버 정정라벨로/곡률 structure-G2 = 학습단계서 분절↓·단차보존). D5 cp=(다)부분레버(라벨 정정전). 검증 must-fix 3·should-fix 5 반영.
...
> ⚡⚡⚡ **D6 step0 (곡면 과분할 청정 재진단) 완료 (2026-06-27, `feat/p2-d6-curved`, 커밋 `105da76`, 미푸시)** — 관찰만, 판정=김휘영. 본보고 `docs/experiments/w_d6_overseg_diag/reports/W_D6_overseg_diag.md`(§0 무결성+§2~4 a/b/c+§6 레버 지시). 그림 `docs/figs/W_D6/`.
...
> 사양·사전등록(LOCKED §5) = `P2_D5_cp_ablation_사양_사전등록_20260626.md`. 본보고 = `docs/experiments/w_d5/reports/W_D5.md`(§0 사전점검 + §2~ cp 판정표 gssem|smrf + §5 기준 대입).
...
> **본보고**: `docs/experiments/w_d4/reports/W_D4.md`(2축 8-way + §6 판정 패키지). **사양·사전등록(LOCKED)**: `P2_D4_사양서_사전등록_20260625.md`(§6 판정기준·§7 cp 사다리). 그림 `docs/figs/W_D4_qual/`.
...
- **⚡ NEXT = 판정(김휘영)**: 사전등록 §6 성공조건 1–5 **충족방향** but valid-solid-dense 회귀 = **부분지지** 신호 → 성공 vs 부분지지 판정. 부분지지면 §7 cp 사다리(cp 이미 발화→불발동) 또는 **valid-solid/위상 후속**(roofer 1.0.0 위상수리 플래그 無). **커밋 "D4" 완료**(`feature/p2-prior-full`, 미푸시; P2_D4_사양서·configs gs_d4_*·run_d4.sh·d4_table.py·d4_qual_figs.py·docs/experiments/w_d4/reports/W_D4.md·figs/W_D4_qual/. results/ data는 gitignore=재생성).
...
> **보고서**: `docs/experiments/w_d_prior_full/reports/W_D_prior_full.md`(본보고·2축·G1) · `W_D_loss_audit.md` · `W_D_followup_audit.md` · `W_D2_D3.md` · `W_D4_precheck.md`. 그림 `docs/figs/W_D_qual/`.
