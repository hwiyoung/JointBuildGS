...
> ⚡⚡⚡⚡⚡⚡⚡⚡⚡⚡⚡⚡⚡⚡ **facet-coverage-inventory — 장면 GS 커버리지 + 면 단위 자료 (2026-06-30, `feat/p2-structure-learn`, 커밋 `facet-coverage-inventory`, 미푸시)** — **읽기 전용**, 관찰만, 판정=김휘영. `docs/W_observability_inventory.md` **§6 추가**.
...
> ⚡⚡⚡⚡⚡⚡⚡⚡⚡⚡⚡⚡⚡ **observability-inventory — 관측가능성 자산 인벤토리 (2026-06-30, `feat/p2-structure-learn`, 커밋 `observability-inventory`, 미푸시)** — **읽기 전용**(재학습·재구성·data/raw 무변경), 관찰만, 판정=김휘영. 본보고 `docs/W_observability_inventory.md`(A~E 표). 새 스크립트 없음(기존 P0/D9~D11 산출 확인만).
...
> ⚡⚡⚡⚡⚡⚡⚡⚡⚡⚡⚡⚡ **complexity-survey — 메트릭 고정 + 복잡도 매핑 (2026-06-30, `feat/p2-structure-learn`, 커밋 `complexity-survey`, 미푸시)** — **재학습 없음**(기존 GS11·ALS·LoD2·Roofer 재사용), 관찰만, 판정=김휘영. 본보고 `docs/W_complexity_survey.md`. 재현 `complexity_metric.py`(Part1)·`complexity_survey.py`(Part2~4).
...
> ⚡⚡⚡⚡⚡⚡⚡⚡⚡⚡⚡ **Phase B / B1 — 자기지도 다시점 평면 일관성(L_mvc) 학습 완료 (2026-06-30, `feat/p2-structure-learn`, 커밋 `phaseB-b1-mvconsist`, 미푸시)** — **재학습 있음**(이 항만 격리), 관찰만, 판정=김휘영. 본보고 `docs/W_phaseB_structure.md`(§0 Step0 + §2 구현 + §0″/§3~5 정정본). 재현 `run_b1.sh`·`overseg_b1_faithfulness.py`(--arms·shared-dz).
