...
SCRIPT_PATH = Path(__file__).with_name("fusion_w1_targets.py")
...
class FusionW1TargetTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        cls.repo_root = SCRIPT_PATH.resolve().parents[3]
        cls.config_path = (
            cls.repo_root
            / "phases/p2-gsjso/configs/fusion_w1_targets_v1.json"
        )
    ...
    def test_height_primary_comes_from_committed_lock(self) -> None:
        lock = json.loads(
            (
                self.repo_root
                / "phases/p2-gsjso/configs/primary4_assembly_validation_v2.json"
    ...
    def test_fixed_repository_outputs_match_exact_regeneration(self) -> None:
        output = (
            self.repo_root
            / "phases/p2-gsjso/runs/20260724_fusion_w1/w1_targets.csv"
        )
