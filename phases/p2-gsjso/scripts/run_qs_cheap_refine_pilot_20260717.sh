...
RUN_REL="phases/p2-gsjso/runs/20260717_qs_cheap_refine_pilot"
RUN="$REPO/$RUN_REL"
...
run_tools python3 phases/p2-gsjso/scripts/qs_cheap_refine_pilot.py prepare \
  > "$RUN/logs/prepare.log" 2>&1

write_status "running" "MLS default refinement"
run_tools python3 phases/p2-gsjso/scripts/overseg_smooth.py \
...
data = json.loads(Path("phases/p2-gsjso/runs/20260717_qs_cheap_refine_pilot/prepared.json").read_text())
...
run_tools python3 phases/p2-gsjso/scripts/qs_cheap_refine_pilot.py finalize \
...
rows = list(csv.DictReader(Path("docs/qs_cheap_refine_pilot.csv").open()))
