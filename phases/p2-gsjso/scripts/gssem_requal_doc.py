...
Reads numbers_{smrf,gssem}.json (gssem_requal_numbers.py output) and writes docs/W_gssem_requal.md:
...
w("\n## §5 재현/출처")
w("- 재-eval: `phases/p2-gsjso/scripts/run_gssem_requal.sh` (백업→gssem eval→ref_rms→numbers→verify; CPU/도커, NO GPU, gs_d5* 미접촉).")
...
(REPO / "docs/W_gssem_requal.md").write_text("\n".join(L) + "\n")
print(f"[done] -> {REPO/'docs/W_gssem_requal.md'}")
