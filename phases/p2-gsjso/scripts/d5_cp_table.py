...
Writes the §5 result section into docs/W_D5.md (replaces the '§2~ 대기' placeholder; keeps §0/§1).
"""
...
def main():
    ...
    doc = M.parent.parent / "docs/W_D5.md"   # results/tum_transfer/.. -> repo? fix below
    doc = REPO / "docs/W_D5.md"
    body = "\n".join(L) + "\n"
    if doc.exists():
        txt = doc.read_text()
        idx = txt.find("## §2~")
