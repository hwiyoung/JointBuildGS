...
RUN_REL="phases/p2-gsjso/runs/20260715_e5_c001_s3ap_phase0_baselines"
...
docker run --rm -i \
  ...
  "$IMAGE" \
  python3 phases/p2-gsjso/scripts/e5_c001_s3ap_phase0_baselines.py "${ARGS[@]}"
...
docker run --rm -i \
  ...
  -w /workspace/JointBuildGS \
  "$IMAGE" \
  python3 phases/p2-gsjso/scripts/e5_c001_s3ap_phase0_roofer_finalize.py \
