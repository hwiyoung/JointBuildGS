...
python3 phases/p2-gsjso/scripts/gssem_requal_numbers.py smrf "$BK/numbers_smrf.json" || { echo "smrf-snapshot FAIL"; echo fail>"$BK/PART1_FAIL"; exit 2; }
...
echo "[$(date '+%T')] EVAL gssem D";  python3 phases/p2-gsjso/scripts/tum_mob_eval.py --configs $CFG_D  --geojson "$GEOJSON" --classifier gssem --out "$OUT/eval_prior_full_gssem.json" > "$BK/eval_D_gssem.log"  2>&1
echo "[$(date '+%T')] EVAL gssem D4"; python3 phases/p2-gsjso/scripts/tum_mob_eval.py --configs $CFG_D4 --geojson "$GEOJSON" --classifier gssem --out "$OUT/eval_d4_gssem.json"         > "$BK/eval_D4_gssem.log" 2>&1
...
echo "[$(date '+%T')] RMS gssem D";  $TOOLS python3 phases/p2-gsjso/scripts/tum_mob_ref_rms.py --arms $ARMS_D  --out "$A/ref_rms_D_gssem.csv"  > "$BK/rms_D_gssem.log"  2>&1
echo "[$(date '+%T')] RMS gssem D4"; $TOOLS python3 phases/p2-gsjso/scripts/tum_mob_ref_rms.py --arms $ARMS_D4 --out "$A/ref_rms_d4_gssem.csv" > "$BK/rms_D4_gssem.log" 2>&1

# ===== 4) snapshot gssem NUMBERS (disk now = gssem) =====
python3 phases/p2-gsjso/scripts/gssem_requal_numbers.py gssem "$BK/numbers_gssem.json"
