...
# Launch: setsid nohup bash phases/p2-gsjso/scripts/run_overnight.sh > results/tum_transfer/mob/overnight.log 2>&1 < /dev/null &
cd "$(dirname "$0")/../../.." || exit 1
...
log "===== TASK A (D12 metric-final) start ====="
{
  ...
  git add docs/experiments/evaluation/w_d12_metric/reports/W_D12_metric_final.md phases/p2-gsjso/scripts/d12_metric_final.py \
...
{
  # Step0 — bucket labels
  $TOOLS python3 phases/p2-gsjso/scripts/d12_buckets.py > "$OUT/overnight_logs/taskB_buckets.log" 2>&1 \
  ...
  if [ ! -f "$OUT/overseg_lever/d12_buckets.csv" ]; then
    ...
    if [ ! -f "$BASE" ]; then
      $TOOLS python3 phases/p2-gsjso/scripts/tum_mob_baselines.py \
    ...
    eval_one(){ # $1=arm $2=npz $3=classifier $4=targets $5=tag
      ...
      python3 phases/p2-gsjso/scripts/tum_mob_eval.py --configs ${arm}=${npz} --geojson "$GEOJSON" \
    ...
    for arm in gs_seed_sparse gs_seed_dense gs_seed_acmp; do
      ...
      for ((i=0;i<NM;i+=CG)); do
        ...
        python3 phases/p2-gsjso/scripts/tum_mob_eval.py --configs ${arm}=$OUT/tsdf_gen8.npz --geojson "$GEOJSON" \
    ...
    git add docs/experiments/evaluation/w_generation_8way/reports/W_generation_8way.md phases/p2-gsjso/scripts/d12_buckets.py \
