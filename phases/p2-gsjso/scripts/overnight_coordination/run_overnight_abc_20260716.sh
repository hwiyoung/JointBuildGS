...
# Launch:
#   setsid nohup bash phases/p2-gsjso/scripts/overnight_coordination/run_overnight_abc_20260716.sh \
...
run_a() {
  ...
  log "A start"
  if ! run_tools python3 scripts/evaluation/quality_score/overnight_qs_rescore.py \
...
run_b() {
  ...
  log "B start"
  if ! run_tools python3 phases/p2-gsjso/scripts/overnight_coordination/overnight_genclose.py prepare \
  ...
  if ! run_tools python3 phases/p2-gsjso/scripts/overnight_coordination/overnight_genclose.py finalize \
...
metrics = list(csv.DictReader(Path("docs/evidence/archive/boundary_map/v1/tables/boundary_map_metrics.csv").open()))
ladder = list(csv.DictReader(Path("docs/evidence/archive/boundary_map/v1/tables/boundary_map_ladder.csv").open()))
confusion = list(csv.DictReader(Path("docs/evidence/archive/boundary_map/v1/tables/boundary_map_confusion.csv").open()))
cases = list(csv.DictReader(Path("docs/evidence/archive/boundary_map/v1/tables/boundary_map_boundary_cases.csv").open()))
...
if manifest["evaluation_population"] != 178 or manifest["learning_runs_started"] != 0:
    raise SystemExit("manifest QA failed")
if not Path("docs/evidence/archive/boundary_map/v1/figs/boundary_map_ladder.png").is_file():
...
PY
  ...
  issue "OVN-C measurement complete: manifest_sha256=$(sha docs/experiments/input-and-alignment/boundary_map/manifests/boundary_map_manifest.json); metrics_sha256=$(sha docs/evidence/archive/boundary_map/v1/tables/boundary_map_metrics.csv); ladder_sha256=$(sha docs/evidence/archive/boundary_map/v1/tables/boundary_map_ladder.csv); learning_runs_started=0; new_inference=MASt3R_correspondence_only"
  if ! commit_paths \
    "OVN-C: measure 178-building boundary map" \
    phases/p2-gsjso/docs/issues.md \
    docs/evidence/archive/boundary_map/v1/tables/boundary_map_metrics.csv \
    docs/evidence/archive/boundary_map/v1/tables/boundary_map_ladder.csv \
    docs/evidence/archive/boundary_map/v1/tables/boundary_map_confusion.csv \
    docs/evidence/archive/boundary_map/v1/tables/boundary_map_boundary_cases.csv \
