...
#   setsid nohup bash phases/p2-gsjso/scripts/overnight_coordination/run_repair_waves_20260718.sh \
...
RUN_REL="phases/p2-gsjso/runs/boundary_and_robustness/20260718_repair_waves"
RUN="$REPO/$RUN_REL"
LOG_DIR="$RUN/logs"
...
PY
  ...
  issue "RW-R1 measurement complete: metrics_sha256=$(sha docs/evidence/archive/boundary_map/v2/tables/boundary_map_v2_metrics.csv); ladder_sha256=$(sha docs/evidence/archive/boundary_map/v2/tables/boundary_map_v2_ladder.csv); manifest_sha256=$(sha docs/experiments/input-and-alignment/boundary_map/manifests/boundary_map_v2_manifest.json); figure_sha256=$(sha docs/evidence/archive/boundary_map/v2/figs/boundary_map_v2_ladder.png); learning_runs_started=0; new_inference=R1-2_crop_pair_and_R1-4_FM_only"
  if ! commit_paths \
    ...
    docs/evidence/archive/boundary_map/v2/tables/boundary_map_v2_confusion.csv \
    docs/experiments/input-and-alignment/boundary_map/tables/boundary_map_v2_boundary_cases.csv \
    docs/evidence/archive/boundary_map/v2/tables/boundary_map_v2_conditional_targets.csv \
    docs/experiments/input-and-alignment/boundary_map/manifests/boundary_map_v2_manifest.json \
    docs/evidence/archive/boundary_map/v2/reports/W_boundary_map_v2_summary_20260718.md \
    docs/evidence/archive/boundary_map/v2/figs \
...
run_r2() {
  ...
  if ! run_tools python3 scripts/evaluation/quality_score/qs_rescore_completeness_panel.py \
...
scores = list(csv.DictReader(Path("docs/experiments/evaluation/qs_rescore/tables/qs_rescore_scores.csv").open()))
...
for path in (
    ...
    Path("docs/experiments/evaluation/qs_rescore/reports/W_qs_rescore_completeness_panel_20260718.md"),
):
    if not path.is_file():
...
PY
  ...
  if ! commit_paths \
    ...
    docs/experiments/evaluation/qs_rescore/tables/qs_rescore_topview_panel.csv \
...
run_r3() {
  ...
  if ! run_tools python3 scripts/evaluation/quality_score/qs_baseline178_rescore.py \
...
scores = list(csv.DictReader(Path("docs/experiments/evaluation/qs_baseline178/tables/qs_baseline178_scores.csv").open()))
summary = list(csv.DictReader(Path("docs/experiments/evaluation/qs_baseline178/tables/qs_baseline178_summary.csv").open()))
manifest = json.loads(Path("docs/experiments/evaluation/qs_baseline178/manifests/qs_baseline178_manifest.json").read_text())
...
PY
  ...
  issue "RW-R3 measurement complete: scores_sha256=$(sha docs/experiments/evaluation/qs_baseline178/tables/qs_baseline178_scores.csv); summary_sha256=$(sha docs/experiments/evaluation/qs_baseline178/tables/qs_baseline178_summary.csv); manifest_sha256=$(sha docs/experiments/evaluation/qs_baseline178/manifests/qs_baseline178_manifest.json); figure_sha256=$(sha docs/figs/qs_baseline178/dense_vs_als_rms_distribution.png); learning_runs_started=0; new_inference_runs=0"
  if ! commit_paths \
...
run_r4() {
  ...
  if ! python3 scripts/evaluation/quality_score/qs_cheap_refine_sweep.py run \
...
scores = list(csv.DictReader(Path("docs/experiments/evaluation/qs_cheap_refine_sweep/tables/qs_cheap_refine_sweep.csv").open()))
summary = list(csv.DictReader(Path("docs/experiments/evaluation/qs_cheap_refine_sweep/tables/qs_cheap_refine_sweep_summary.csv").open()))
manifest = json.loads(Path("docs/experiments/evaluation/qs_cheap_refine_sweep/manifests/qs_cheap_refine_sweep_manifest.json").read_text())
...
PY
  ...
  issue "RW-R4 measurement complete: scores_sha256=$(sha docs/experiments/evaluation/qs_cheap_refine_sweep/tables/qs_cheap_refine_sweep.csv); summary_sha256=$(sha docs/experiments/evaluation/qs_cheap_refine_sweep/tables/qs_cheap_refine_sweep_summary.csv); manifest_sha256=$(sha docs/experiments/evaluation/qs_cheap_refine_sweep/manifests/qs_cheap_refine_sweep_manifest.json); figure_sha256=$(sha docs/figs/qs_cheap_refine_sweep/parameter_grid.png); learning_runs_started=0; new_inference_runs=0"
  if ! commit_paths \
    "RW-R4: sweep C001 cheap-refinement parameters" \
    phases/p2-gsjso/docs/issues.md \
    docs/experiments/evaluation/qs_cheap_refine_sweep/tables/qs_cheap_refine_sweep.csv \
