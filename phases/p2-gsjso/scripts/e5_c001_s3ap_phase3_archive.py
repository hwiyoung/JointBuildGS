...
DEFAULT_CONFIG = REPO / "phases/p2-gsjso/configs/e5_c001_s3ap_phase3_archive_lock.json"
...
def validate_executed_controller_source(repo: Path = REPO) -> str:
    ...
    current = sha256_file(repo_path(
        "phases/p2-gsjso/scripts/e5_c001_s3ap_phase3_archive.py", repo,
    ))
            ...
            if arm == "a0":
                expected_seed_path = str(
                    (phase2_lock or {}).get("sources", {}).get(
                        ...
                        "phases/p2-gsjso/runs/20260715_e5_c001_s3ap_phase1_seedprep/seeds/DEBY_LOD2_{building}_p0_surface_seed.npz",
                    )
