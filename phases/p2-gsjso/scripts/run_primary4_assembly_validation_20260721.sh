...
RUN_REL="phases/p2-gsjso/runs/20260721_primary4_assembly_validation"
RUN="$REPO/$RUN_REL"
DRIVER_REL="phases/p2-gsjso/runs/20260721_primary4_assembly_validation_driver"
DRIVER="$REPO/$DRIVER_REL"
LOG_DIR="$DRIVER/logs"
STATUS="$DRIVER/status.json"
LOCK="$DRIVER/driver.lock"
ISSUES="$REPO/docs/issues.md"
SCRIPT="phases/p2-gsjso/scripts/primary4_assembly_validation.py"
QA_SCRIPT="phases/p2-gsjso/scripts/primary4_assembly_validation_qa.py"
...
finalize_bundle() {
  ...
  counts="$(python3 - <<'PY'
import csv
rows=list(csv.DictReader(open('docs/primary4_assembly_validation_measurements.csv', newline='', encoding='utf-8')))
...
PY
  ...
  issue "$TASK measurement bundle complete: rows=4; $counts; measurements_sha256=$(sha docs/primary4_assembly_validation_measurements.csv); summary_sha256=$(sha docs/W_primary4_assembly_validation_summary_20260721.md); manifest_sha256=$(sha docs/primary4_assembly_validation_manifest.json); independent_QA=passed; learning_runs_started=0; new_inference_runs=0; image_inputs_used=0; gpu_used=false"
