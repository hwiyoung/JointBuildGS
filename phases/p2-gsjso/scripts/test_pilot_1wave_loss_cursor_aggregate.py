...
SCRIPT = Path(__file__).with_name("pilot_1wave_loss_cursor_aggregate.py")
...
class PilotOneWaveLossCursorAggregateTests(unittest.TestCase):
    def setUp(self) -> None:
        self.temporary = tempfile.TemporaryDirectory(
            prefix=".p1w_loss_cursor_", dir=Path(__file__).resolve().parent
        )
    ...
    def test_output_fields_match_scoring_contract_without_importing_scorer(self) -> None:
        scoring_path = Path(__file__).with_name("pilot_1wave_scoring.py")
