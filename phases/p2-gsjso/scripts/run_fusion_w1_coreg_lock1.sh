...
CONFIG="phases/p2-gsjso/configs/fusion_w1_coreg_lock1.json"
...
run_tools() {
  docker run --rm \
    ...
    "$TOOLS_IMAGE" \
    python3 phases/p2-gsjso/scripts/fusion_w1_coreg_lock1.py \
    --config "$CONFIG" "$@"
}
...
case "${1:-}" in
  ...
  test)
    docker run --rm \
      ...
      phases/p2-gsjso/scripts/test_fusion_w1_coreg_lock1.py
    ;;
  *)
    echo "usage: $0 {prepare-controls|prepare-als|measure|verify|test}" >&2
    exit 2
    ;;
esac
