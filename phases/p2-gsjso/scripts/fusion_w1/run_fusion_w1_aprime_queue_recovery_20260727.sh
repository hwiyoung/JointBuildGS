#!/usr/bin/env bash
# Docker-only recovery queue controller; keeps the locked producer HEAD intact.
set -Eeuo pipefail

REPO_ROOT="$(git rev-parse --show-toplevel)"
cd "$REPO_ROOT"

CONFIG="phases/p2-gsjso/configs/fusion_w1/fusion_w1_aprime_queue_20260726.json"
DRIVER="phases/p2-gsjso/scripts/fusion_w1/fusion_w1_aprime_queue_recovery_20260727.py"
TEST="tests/fusion_w1/test_fusion_w1_aprime_queue_recovery_20260727.py"
TRAINING_WRAPPER="phases/p2-gsjso/scripts/fusion_w1/run_fusion_w1_aprime_training_20260726.sh"
READOUT_WRAPPER="phases/p2-gsjso/scripts/fusion_w1/run_fusion_w1_aprime_readout_20260726.sh"
QUEUE_ROOT="phases/p2-gsjso/runs/fusion_w1/20260726_fusion_w1_aprime/unattended_queue"
...
run_queue() {
  ...
  acquire_queue_lock
  run_tools "$DRIVER" --config "$CONFIG" publish-controller
...
case "${1:-}" in
  ...
  rehydrate-smoke)
    run_tools "$DRIVER" --config "$CONFIG" publish-controller
    run_tools "$DRIVER" --config "$CONFIG" rehydrate-valid-archive \
      --stage-key smoke_barrier --stage-entry-order 1 --attempt 1
    ;;
  run)
    run_queue
    ;;
  ...
  next)
    ...
    ;;
  *)
    echo "usage: $0 {test|rehydrate-smoke|run|snapshot|next}" >&2
    exit 2
    ;;
esac
