...
# Launch detached:  setsid nohup bash phases/p2-gsjso/scripts/run_mob_v6.sh > results/tum_transfer/mob/v6.log 2>&1 &
...
if [ ! -f "$OUT/baselines.json" ]; then
  ...
  python3 phases/p2-gsjso/scripts/tum_mob_baselines.py > "$OUT/baselines.log" 2>&1 || \
