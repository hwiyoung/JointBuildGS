...
def extract_footprint(wall_groups, centers):
    ...
    # When walls have few primitives (e.g., GT test = 1 prim/wall), per-wall
    # t_min/t_max collapse to a single point. Need a global margin that covers
    # the distance from wall centroid to corners, which is at most building_size/2.
    global_margin = max(5.0, building_size * 0.6)
...
def _snap_face_to_plane(verts):
    """Project all face vertices onto best-fit plane via SVD.

    Eliminates val3dity code 203 (non-planar face) errors caused by
    accumulated numerical error when a face has >= 4 vertices.
    """
    verts = np.asarray(verts, dtype=np.float64)
    if verts.shape[0] < 3:
        return verts
    c = verts.mean(axis=0)
    u, s, vh = np.linalg.svd(verts - c, full_matrices=False)
    n = vh[-1]  # plane normal = last right singular vector
    # Project each vertex onto plane: v' = v - ((v - c) · n) * n
    offset = (verts - c) @ n
    snapped = verts - offset[:, None] * n
    return snapped
...
def faces_to_cityjson(faces, building_id, out_dir, scale=0.0001):
    ...
    Snaps each face's vertices to its best-fit plane (SVD) to guarantee
    planarity (avoids val3dity code 203).
    """
    import json, os

    # Snap every face to its best-fit plane
    for f in faces:
        f['vertices'] = _snap_face_to_plane(f['vertices'])
