...
source_relative_root="phase-payloads/p2/c3_tsdf_roof_diagnostic_v1/P2-C3-TSDF-ROOF-DIAGNOSTIC-RENDER-RECOVERY-v1"
relative_root="phase-payloads/p2/c3_tsdf_roof_diagnostic_v1/P2-C3-TSDF-ROOF-DIAGNOSTIC-REPORT-RECOVERY-v1"
source_root="${artifact_root}/${source_relative_root}"
...
if [[ ! -f "${source_root}/qualitative/index_v1.json" ]]; then
  echo "completed render output is missing" >&2
  exit 2
fi
if [[ -e "${output_root}" ]]; then
  echo "add-once report recovery namespace already exists: ${output_root}" >&2
  exit 2
fi

docker run --rm --network none \
  -v "${artifact_root}:/artifacts/JointBuildGS" \
  "${image}" sh -lc "install -d -o ${host_uid} -g ${host_gid} -m 0755 '/artifacts/JointBuildGS/${relative_root}/reports' '/artifacts/JointBuildGS/${relative_root}/control'"
...
docker run --rm --network none \
  ...
  "${image}" python -B -m scripts.p2.c3_tsdf_roof_diagnostic_v1.finalize prepare \
    --source-root "/artifacts/JointBuildGS/${source_relative_root}" \
