...
config_container="/workspace/JointBuildGS/configs/p2/e4_e6_redesign_s3_v1/run_v1.json"
task_name="P2-E4-E6-REDESIGN-S3-v1"
task_rel="phase-payloads/p2/e4_e6_redesign_s3_v1/${task_name}"
...
if [[ ! -f "${partial_root}/control/prepared_v1.json" ]]; then
  tools_run "python -B -m scripts.p2.e4_e6_redesign_s3_v1.run --config '${config_container}' prepare --output-root /task --artifact-root /artifacts/JointBuildGS --repo-root /workspace/JointBuildGS --scope full199"
fi
mkdir -p "${partial_root}/control/torch_extensions"

for condition_id in E4_V2_STATIC E5_V2_F1; do
  work="${partial_root}/work/${condition_id}"

  if [[ ! -f "${work}/fused_surface_receipt.json" ]]; then
    project_run python -B -m scripts.p2.e4_e6_redesign_s3_v1.run \
  ...
  if [[ ! -f "${work}/classified_scene_receipt.json" ]]; then
    ...
    tools_run "python -B -m scripts.p2.e4_e6_redesign_s3_v1.run --config '${config_container}' verify-classified --output-root /task --condition-id '${condition_id}'"
  fi
  ...
  if [[ ! -f "${work}/roofer_terminal.json" ]]; then
    ...
    exit_code=$?
    set -e
    tools_run "python -B -m scripts.p2.e4_e6_redesign_s3_v1.run --config '${config_container}' record-roofer --output-root /task --condition-id '${condition_id}' --exit-code '${exit_code}' --runtime-seconds '$((SECONDS - start_seconds))'"
  fi
done

tools_run "python -B -m scripts.p2.e4_e6_redesign_s3_v1.run --config '${config_container}' finalize --output-root /task"
