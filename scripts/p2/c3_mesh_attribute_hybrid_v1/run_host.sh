...
relative_root="phase-payloads/p2/c3_mesh_attribute_hybrid_v1/P2-C3-MESH-ATTRIBUTE-HYBRID-v1"
...
if [[ -n "$(git -C "${repo_root}" status --porcelain=v1 --untracked-files=all)" ]]; then
  echo "mesh attribute hybrid execution requires a clean source checkout" >&2
  exit 2
fi
if [[ "$(git -C "${repo_root}" rev-parse HEAD)" != "$(git -C "${repo_root}" rev-parse origin/main)" ]]; then
  echo "mesh attribute hybrid execution requires HEAD=origin/main" >&2
  exit 2
fi
...
docker run --rm --network none --user "${host_uid}:${host_gid}" \
  ...
  -w /workspace/JointBuildGS "${image}" python -B -m scripts.p2.c3_mesh_attribute_hybrid_v1.render \
  --output-root "/artifacts/JointBuildGS/${relative_root}" \
  --artifact-root /artifacts/JointBuildGS --source-commit "${source_commit}"

echo "C3 mesh attribute hybrid complete: ${output_root}"
