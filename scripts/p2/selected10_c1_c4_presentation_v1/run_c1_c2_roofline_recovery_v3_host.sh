...
artifact_root="${1:?usage: run_c1_c2_roofline_recovery_v3_host.sh ARTIFACT_ROOT PROJECT_IMAGE SOURCE_COMMIT RUN_ID}"
project_image="${2:?missing project image}"
source_commit="${3:?missing source commit}"
run_id="${4:?missing run ID}"
source_rel="phase-payloads/p2/selected10_c1_c4_presentation_v1/P2-SELECTED10-C1-C2-ORACLE-PRESENTATION-RECOVERY-v2"
target_rel="phase-payloads/p2/selected10_c1_c4_presentation_v1/P2-SELECTED10-C1-C2-ORACLE-ROOFLINE-RECOVERY-v3"
...
[[ -d "${source_root}" && ! -L "${source_root}" ]] || { echo "closed v2 source missing" >&2; exit 2; }
[[ ! -e "${final_root}" && ! -e "${partial_root}" ]] || { echo "fresh add-once v3 namespace required" >&2; exit 2; }
...
echo "selected10 C1/C2 corrected-roofline v3 complete without Roofer: ${final_root}"
