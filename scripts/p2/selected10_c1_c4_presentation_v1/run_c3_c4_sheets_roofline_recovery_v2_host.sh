...
artifact_root="${1:?usage: run_c3_c4_sheets_roofline_recovery_v2_host.sh ARTIFACT_ROOT PROJECT_IMAGE SOURCE_COMMIT RUN_ID}"
project_image="${2:?missing project image}"
source_commit="${3:?missing source commit}"
run_id="${4:?missing run ID}"
task_rel="phase-payloads/p2/selected10_c1_c4_presentation_v1/P2-SELECTED10-C3-1-C3-2-C4-SHEETS-ROOFLINE-RECOVERY-v2"
...
[[ ! -e "${task_root}" ]] || { echo "fresh add-once C3/C4 roofline-recovery namespace required" >&2; exit 2; }
mkdir -p "${task_root}"
...
docker run --rm --network none --entrypoint /opt/conda/bin/python \
  ...
  scripts/p2/selected10_c1_c4_presentation_v1/c3_c4_sheets.py \
    ...
    --config configs/p2/selected10_c1_c4_presentation_v1/c3_c4_sheets_v2.json

echo "selected10 C3-1/C3-2/C4 roofline-recovery sheets complete: ${task_root}"
