...
artifact_root="${1:?usage: run_single_host.sh ARTIFACT_ROOT PROJECT_IMAGE SOURCE_COMMIT RUN_ID BUILDING_ID}"
project_image="${2:?missing project image}"
source_commit="${3:?missing source commit}"
run_id="${4:?missing run ID}"
building_id="${5:?missing building ID}"
stable_suffix="${building_id#DEBY_LOD2_}"
task_rel="phase-payloads/p2/selected10_c1_c4_presentation_v1/P2-SELECTED10-${stable_suffix}-PILOT-v1"
...
[[ ! -e "${task_root}" ]] || { echo "fresh add-once single-building namespace required" >&2; exit 2; }
mkdir -p "${task_root}"
...
docker run --rm --network none --entrypoint python \
  ...
  scripts/p2/selected10_c1_c4_presentation_v1/render.py \
    ...
    --validation-building-id "${building_id}"

echo "single-building selected10 pilot complete: ${task_root}"
