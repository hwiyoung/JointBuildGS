...
artifact_root="${1:?usage: run_c3_c4_sheets_host.sh ARTIFACT_ROOT PROJECT_IMAGE SOURCE_COMMIT RUN_ID}"
project_image="${2:?missing project image}"
source_commit="${3:?missing source commit}"
run_id="${4:?missing run ID}"
task_rel="phase-payloads/p2/selected10_c1_c4_presentation_v1/P2-SELECTED10-C3-1-C3-2-C4-SHEETS-v1"
...
[[ ! -e "${task_root}" ]] || { echo "fresh add-once C3/C4 namespace required" >&2; exit 2; }
mkdir -p "${task_root}"
...
echo "selected10 C3-1/C3-2/C4 sheets complete: ${task_root}"
