...
def _atlas_face_colors(mesh: o3d.geometry.TriangleMesh, bounds: tuple[float, float, float, float], rgba: np.ndarray, support: np.ndarray, mode: str) -> np.ndarray:
...
def _panel(path: Path, *, mesh: o3d.geometry.TriangleMesh, reference: Any, view: str, ground_z: float, zlim: tuple[float, float], bounds: tuple[float, float, float, float], rgba: np.ndarray, support: np.ndarray, mode: str, title: str) -> None:
    ...
    note = "current RGB planar UV roof texture; gray=UNOBSERVED; walls/ground excluded" if mode == "TEXTURE" else "distinct current RGB view support; gray=0 views"
...
def run(output_root: Path, artifact_root: Path, repo_root: Path, source_commit: str) -> dict[str, Any]:
    ...
    mesh_root=resolve_artifact(artifact_root,config["source"]["mesh_relative_root"],"mesh root")
    data_root=resolve_artifact(artifact_root,config["source"]["colmap_relative_root"],"COLMAP")
    lod2=resolve_artifact(artifact_root,config["source"]["lod2_relative_path"],"LoD2")
    plan=json.loads(resolve_artifact(artifact_root,config["source"]["shared_view_plan_relative_path"],"shared view plan").read_text())["plan"]
    references=load_building_references(lod2,config["scope"]["building_ids"])
    dataset=ColmapDataset(data_root,downscale=float(config["texture"]["image_downscale"]),load_depth=True,load_normal=False,load_semantic=False,visible_views=_visible_names(config,repo_root))
    frame_by_name={frame.name:(index,frame) for index,frame in enumerate(dataset.frames)}
    texture_records=[]; cases=[]
    for stable_id in config["scope"]["building_ids"]:
        reference=references[stable_id]; pad=float(config["texture"]["atlas_footprint_padding_m"]); bounds=reference.footprint.buffer(pad).bounds
        view_rows=plan[stable_id][:int(config["texture"]["maximum_views_per_building"])]
        frames=[(frame_by_name[row["name"]][1],dataset[frame_by_name[row["name"]][0]]) for row in view_rows]
        outputs={}; zvalues=[]
        for condition in config["scope"]["condition_ids"]:
            for method in config["scope"]["mesh_methods"]:
                source_path=_mesh_path(mesh_root,condition,stable_id,method); mesh=o3d.io.read_triangle_mesh(str(source_path)); mesh.compute_vertex_normals(); zvalues.extend(np.asarray(mesh.vertices)[:,2].tolist())
                rgba,support,roof_mesh,receipt=_bake_texture(mesh,bounds,frames,config)
                root=output_root/f"textures/{condition}/{stable_id}/{method.lower()}"; texture_path=root/"roof_texture_current_rgb_v1.png"; support_path=root/"roof_texture_view_support_v1.png"
                ...
                write_new(texture_path,encoded.tobytes())
                max_support=max(int(np.max(support)),1); support_rgb=np.rint(plt.get_cmap("plasma")(support/max_support)*255).astype(np.uint8); support_rgb[support==0]=np.asarray([140,140,140,255],np.uint8)
                ok,encoded=cv2.imencode(".png",cv2.cvtColor(support_rgb,cv2.COLOR_RGBA2BGRA));
                if not ok: raise RuntimeError("support encode failed")
                write_new(support_path,encoded.tobytes())
                obj_path=root/"roof_textured_v1.obj"; mtl_path=root/"roof_textured_v1.mtl"
                write_new(obj_path,_obj_bytes(roof_mesh,bounds,mtl_path.name)); write_new(mtl_path,("newmtl roof_texture\nKd 1 1 1\nmap_Kd roof_texture_current_rgb_v1.png\n").encode("ascii"))
                receipt.update({"schema":"jointbuildgs.c3_roof_texture_bake_record.v1","condition_id":condition,"stable_id":stable_id,"mesh_method":method,"source_mesh":file_record(source_path,mesh_root),"texture":file_record(texture_path,output_root),"support":file_record(support_path,output_root),"obj":file_record(obj_path,output_root),"mtl":file_record(mtl_path,output_root),"uv_bounds_epsg25832":list(map(float,bounds)),"gt_footprint_xy_used_for_atlas_bounds":True,"source_mesh_reconstructed":False,"textured_output_is_top_surface_subset":True})
                receipt_path=root/"roof_texture_receipt_v1.json"; write_new(receipt_path,canonical_json_bytes(receipt)); texture_records.append(receipt); outputs[(condition,method)]=(roof_mesh,rgba,support)
        zlim=(float(np.quantile(zvalues,0.001)-1.5),float(np.quantile(zvalues,0.999)+1.5)); ground_z=zlim[0]+1.0; rows=[]; panels=[]
        for method in config["scope"]["mesh_methods"]:
            for mode in ("TEXTURE","SUPPORT"):
                paths=[]
                for condition in config["scope"]["condition_ids"]:
                    mesh,rgba,support=outputs[(condition,method)]
                    for view in VIEWS:
                        path=output_root/f"qualitative/{stable_id}/panels/{method.lower()}_{mode.lower()}_{condition}_{view.lower()}.png"
                        _panel(path,mesh=mesh,reference=reference,view=view,ground_z=ground_z,zlim=zlim,bounds=bounds,rgba=rgba,support=support,mode=mode,title=f"{condition} | {method} | ROOF TEXTURE | {mode} | {view}")
                        paths.append(path); panels.append(file_record(path,output_root))
                rows.append((f"{method}\n{mode}",paths))
        sheet=output_root/f"qualitative/{stable_id}/case_sheet_roof_texture_v1.png"; _sheet(sheet,stable_id,rows)
        cases.append({"stable_id":stable_id,"row_count":4,"column_count":8,"panel_count":len(panels),"case_sheet":file_record(sheet,output_root),"panels":panels})
    index={"schema":"jointbuildgs.c3_roof_texture_bake_index.v1","status":"COMPLETE_CURRENT_RGB_ROOF_ONLY_TEXTURES","source_commit":source_commit,"case_count":3,"texture_bake_count":len(texture_records),"panel_count":sum(c["panel_count"] for c in cases),"cases":cases,"execution_counters":{"gs_training_invocations":0,"checkpoint_render_extractions":0,"poisson_reconstructions":0,"tsdf_reconstructions":0,"roof_texture_bakes":len(texture_records),"roofer_invocations":0,"g2_invocations":0,"metric_recomputations":0,"c4_c5_accesses":0},"official_G3_G4_PASS_usable":None,"scientific_verdict":None}
    write_new(output_root/"qualitative/index_v1.json",canonical_json_bytes(index))
    report="# C3 roof-only current RGB texture bake\n\nPoisson과 TSDF roof mesh 각각에 exact current RGB 12-view를 planar UV atlas로 투영했다. Mesh self-z-buffer와 MVS depth occlusion을 적용하고, 관측되지 않은 texel은 회색/투명으로 남겼다. Wall과 ground texture는 만들지 않았다. 이 texture는 geometry metric 또는 scientific verdict가 아니다.\n"
    write_new(output_root/"reports/technical_report_ko_v1.md",report.encode("utf-8")); links="".join(f'<section><h2>{html.escape(c["stable_id"])}</h2><a href="../{c["case_sheet"]["path"]}"><img src="../{c["case_sheet"]["path"]}"></a></section>' for c in cases); write_new(output_root/"reports/case_index.html",("<!doctype html><meta charset=utf-8><style>img{width:100%}</style><h1>C3 roof textures</h1>"+links).encode())
    returned={"schema":"jointbuildgs.c3_roof_texture_bake_technical_return.v1","status":"RETURNED_LOCAL_COMPLETE_ROOF_TEXTURE_DIAGNOSTIC","source_commit":source_commit,"generated_at":datetime.now(timezone.utc).isoformat().replace("+00:00","Z"),"case_count":3,"texture_bake_count":12,"panel_count":index["panel_count"],"execution_counters":index["execution_counters"],"official_G3_G4_PASS_usable":None,"scientific_verdict":None}; write_new(output_root/"control/technical_return_v1.json",canonical_json_bytes(returned))
    manifest={"schema":"jointbuildgs.c3_roof_texture_bake_artifact_manifest.v1","status":"COMPLETE_HASHED_MATERIAL_PAYLOAD","source_commit":source_commit,"records":_records(output_root),"scientific_verdict":None}; manifest["record_count"]=len(manifest["records"]); write_new(output_root/"control/artifact_manifest_v1.json",canonical_json_bytes(manifest))
    verified={"schema":"jointbuildgs.local_technical_200_verified.v1","status":"200-VERIFIED_LOCAL_SELF_CHECK","checks":{"case_count_3":len(cases)==3,"texture_bakes_12":len(texture_records)==12,"panels_96":index["panel_count"]==96,"walls_excluded":all(not r["wall_texture_created"] for r in texture_records),"no_inpainting":all(not r["unobserved_texels_inpainted"] for r in texture_records),"prohibited_counters_zero":all(index["execution_counters"][k]==0 for k in ("gs_training_invocations","checkpoint_render_extractions","poisson_reconstructions","tsdf_reconstructions","roofer_invocations","g2_invocations","metric_recomputations","c4_c5_accesses")),"scientific_verdict_null":index["scientific_verdict"] is None},"manifest":file_record(output_root/"control/artifact_manifest_v1.json",output_root),"scientific_verdict":None}
    if not all(verified["checks"].values()): raise RuntimeError("roof texture verification failed")
    write_new(output_root/"control/200-verified.local_v1.json",canonical_json_bytes(verified)); closed={"schema":"jointbuildgs.local_technical_300_closed.v1","status":"300-CLOSED_LOCAL_ROOF_TEXTURE_DIAGNOSTIC","technical_return":file_record(output_root/"control/technical_return_v1.json",output_root),"verified":file_record(output_root/"control/200-verified.local_v1.json",output_root),"manifest":file_record(output_root/"control/artifact_manifest_v1.json",output_root),"official_G3_G4_PASS_usable":None,"scientific_verdict":None}; write_new(output_root/"control/300-closed.local_v1.json",canonical_json_bytes(closed)); return closed
