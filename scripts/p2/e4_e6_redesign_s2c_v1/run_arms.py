...
COMMON = REPO / "configs/p2/e4_e6_redesign_s2c_v1/s2c_v1.yaml"
TASK_ID = "P2-E4-E6-REDESIGN-S2C-v1"
TASK_ROOT = ARTIFACT_ROOT / "phase-payloads/p2/e4_e6_redesign_s2c_v1" / TASK_ID
...
def _isolated_record_operation(label: str, argv: list[str], rc: int, started: str, ended: str) -> None:
    ...
    if not path.is_file():
        atomic_json(path, {"schema": "jointbuildgs.p2.e4_e6_redesign_s2c_v1.provenance.v1", "task_id": TASK_ID, "commands": [], "return_codes": [], "scientific_verdict": None})
...
def main() -> None:
    ...
    prior_receipt = TASK_ROOT / "control/200-s2c-prior-preflight-passed.json"
    ...
    atomic_json(TASK_ROOT / f"control/gpu{args.gpu}_training_summary.json", {
        "schema": "jointbuildgs.p2.e4_e6_redesign_s2c_v1.training_summary.v1",
        "task_id": TASK_ID,
        "gpu": args.gpu,
