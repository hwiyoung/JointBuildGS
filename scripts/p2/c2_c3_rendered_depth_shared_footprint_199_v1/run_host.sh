...
artifact_root="${1:?usage: run_host.sh ARTIFACT_ROOT pilot10|full199 [GPU_INDEX]}"
scope="${2:?usage: run_host.sh ARTIFACT_ROOT pilot10|full199 [GPU_INDEX]}"
gpu_index="${3:-0}"
...
config_container="/workspace/JointBuildGS/configs/p2/c2_c3_rendered_depth_shared_footprint_199_v1/run_v1.json"

case "${scope}" in
  pilot10) task_name="P2-C2-C3-RENDERED-DEPTH-SHARED-FOOTPRINT-PILOT10-v1" ;;
  full199) task_name="P2-C2-C3-RENDERED-DEPTH-SHARED-FOOTPRINT-199-v1" ;;
  *) echo "scope must be pilot10 or full199" >&2; exit 2 ;;
esac
task_rel="phase-payloads/p2/c2_c3_rendered_depth_shared_footprint_199_v1/${task_name}"
...
if [[ ! -f "${partial_root}/control/prepared_v1.json" ]]; then
  tools_run "python -B -m scripts.p2.c2_c3_rendered_depth_shared_footprint_199_v1.run --config '${config_container}' prepare --output-root /task --artifact-root /artifacts/JointBuildGS --repo-root /workspace/JointBuildGS --scope '${scope}'"
fi
mkdir -p "${partial_root}/control/torch_extensions"

for condition_id in C3_1 C3_2; do
  work="${partial_root}/work/${condition_id}"
  if [[ ! -f "${work}/fused_surface_receipt.json" ]]; then
    project_run python -B -m scripts.p2.c2_c3_rendered_depth_shared_footprint_199_v1.run \
  ...
  if [[ ! -f "${work}/classified_scene_receipt.json" ]]; then
    [[ ! -e "${work}/classified_scene.laz" ]] || { echo "unsealed classified scene refuses reuse: ${condition_id}" >&2; exit 2; }
    ...
    tools_run "python -B -m scripts.p2.c2_c3_rendered_depth_shared_footprint_199_v1.run --config '${config_container}' verify-classified --output-root /task --condition-id '${condition_id}'"
  fi
  if [[ ! -f "${work}/roofer_terminal.json" ]]; then
    [[ ! -e "${work}/roofer_output" ]] || { echo "unsealed Roofer output refuses duplicate execution: ${condition_id}" >&2; exit 2; }
    ...
    exit_code=$?
    set -e
    tools_run "python -B -m scripts.p2.c2_c3_rendered_depth_shared_footprint_199_v1.run --config '${config_container}' record-roofer --output-root /task --condition-id '${condition_id}' --exit-code '${exit_code}' --runtime-seconds '$((SECONDS - start_seconds))'"
  fi
done

tools_run "python -B -m scripts.p2.c2_c3_rendered_depth_shared_footprint_199_v1.run --config '${config_container}' finalize --output-root /task"
mv -- "${partial_root}" "${final_root}"
echo "rendered-depth C3 shared-footprint diagnostic complete: ${final_root}"
