...
task_rel="phase-payloads/p2/utarget199_c1_c4_matrix_v1/P2-UTARGET199-C1-C4-MATRIX-v1"
...
[[ ! -e "${task_root}" ]] || { echo "fresh add-once matrix namespace required" >&2; exit 2; }
mkdir -p "${task_root}"
...
docker run --rm --network none --entrypoint python \
  ...
  scripts/p2/utarget199_c1_c4_matrix_v1/render.py \
    --output-root /task --artifact-root /artifacts/JointBuildGS \
    --source-commit "${source_commit}" --run-id "${run_id}"

echo "U_target199 C1-C4 attached-style matrix complete: ${task_root}"
