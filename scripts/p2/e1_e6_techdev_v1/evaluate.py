...
def screenshots(run_root: Path, condition: str, report_assets: Path) -> list[str]:
    report_assets.mkdir(parents=True,exist_ok=True); cloud=np.asarray(o3d.io.read_point_cloud(str(run_root/"pointcloud/fused_75ptm2.ply")).voxel_down_sample(1.0).points); city=cityjson_vertices(run_root/"roofer/assembled.city.json"); paths=[]
