...
artifact_root="${1:?usage: run_host.sh ARTIFACT_ROOT [REPLAY_LABEL]}"
replay_label="${2:-}"
...
canonical_task_rel="phase-payloads/p2/c1_c2_shared_footprint_199_v1/P2-C1-C2-SHARED-FOOTPRINT-199-v1"
if [[ -n "${replay_label}" ]]; then
  [[ "${replay_label}" =~ ^[A-Za-z0-9][A-Za-z0-9._-]{0,63}$ ]] || { echo "invalid replay label" >&2; exit 2; }
  task_rel="phase-payloads/p2/c1_c2_shared_footprint_199_v1/P2-C1-C2-SHARED-FOOTPRINT-199-v1-replay-${replay_label}"
else
...
eligible_total="$(( $(wc -l < "${partial_root}/freeze/execution_units_v1.tsv") - 1 ))"
eligible_index=0
while IFS=$'\t' read -r operation_id work_relative; do
  ...
  if [[ -f "${work}/roofer_terminal_v1.json" ]]; then
    echo "[${eligible_index}/${eligible_total}] reuse terminal ${operation_id}"
    continue
  fi
  ...
  mkdir "${work}/out"
  echo "[${eligible_index}/${eligible_total}] Roofer ${operation_id}"
...
project_run finalize --output-root /task
audit_args=(
...
echo "shared-footprint C1/C2 199 complete: ${final_root}"
