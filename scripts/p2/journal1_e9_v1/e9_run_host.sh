#!/usr/bin/env bash
# E9 host driver: conflict-aware selective union through the sealed chain,
# for delta in {0, 0.5, 1.0}, evaluated on the confirmed 93 buildings.
# Non-confirmatory technical development; scientific_verdict stays null.
set -euo pipefail

repo_root="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)"
artifact_root="${1:?usage: e9_run_host.sh ARTIFACT_ROOT}"
...
phase_rel="phase-payloads/p2/journal1_e9_v1"
phase_root="${artifact_root}/${phase_rel}"
cont_phase="/artifacts/JointBuildGS/${phase_rel}"
runs_root="${phase_root}/P2-JOURNAL1-E9-v1/runs"
config="${repo_root}/configs/p2/journal1_e9_v1/e9_union_v1.json"
...
while IFS='|' read -r label dx dz mode; do
  root_h="${runs_root}/${label}"
  root_c="${cont_phase}/P2-JOURNAL1-E9-v1/runs/${label}"
  work="${root_h}/work/E9"
  mkdir -p "${root_h}"
  echo "[E9] === ${label} (dx=${dx} dz=${dz} mode=${mode}) ==="

  if [[ ! -f "${root_h}/control/prepared_a2_v1.json" ]]; then
    project_run python -B -m scripts.p2.journal1_e9_v1.e9_build prepare \
      --output-root "${root_c}" --artifact-root /artifacts/JointBuildGS
  fi

  if [[ ! -f "${work}/fused_surface_receipt.json" ]]; then
    project_run python -B -m scripts.p2.journal1_e9_v1.e9_build fuse \
      --output-root "${root_c}" --artifact-root /artifacts/JointBuildGS \
      --delta-xy-east-m "${dx}" --delta-z-m "${dz}" --rule-mode "${mode}"
  fi
  ...
  if [[ ! -f "${work}/classified_scene_receipt.json" ]]; then
    ...
    tools_run "pdal pipeline '${root_c}/work/E9/classification_pipeline.json' >'${root_c}/work/E9/classification.log' 2>&1"
    tools_run "python -B -m scripts.p2.journal1_e9_v1.e9_build verify-classified --output-root '${root_c}'"
  fi
  ...
  if [[ ! -f "${work}/roofer_terminal.json" ]]; then
    ...
    timeout 14400 docker run --rm --network none --cpus 12 --memory 64g --pids-limit 4096 \
      ...
      "work/E9/classified_scene.laz" freeze/shared_footprints.geojson \
      "work/E9/roofer_output" >"${work}/roofer.log" 2>&1
    exit_code=$?
    set -e
    tools_run "python -B -m scripts.p2.journal1_e9_v1.e9_build record-roofer --output-root '${root_c}' --exit-code '${exit_code}' --runtime-seconds '$((SECONDS - start_seconds))'"
  fi

  if [[ ! -f "${root_h}/control/finalized_a2_v1.json" ]]; then
    tools_run "python -B -m scripts.p2.journal1_e9_v1.e9_build finalize --output-root '${root_c}'"
  fi

  if [[ ! -f "${root_h}/assets_roofer_input/E9/receipt.json" ]]; then
    project_run python -B -m scripts.p2.journal1_e9_v1.e9_build crops --output-root "${root_c}"
  fi

  if [[ ! -f "${root_h}/evaluation/receipt.json" ]]; then
    project_run python -B scripts/p2/journal1_phase_d_v1/d1_eval_config.py \
      --run-root "${root_c}" --arm "${label}" --condition E9
  ...
  echo "[E9] ${label} complete"
done < <(python3 -c "
import json
cfg = json.load(open('${config}'))
for r in cfg['runs']:
    print(f\"{r['label']}|{r['dx']}|{r['dz']}|{r['mode']}\")
")

echo "E9 selective-union runs complete: ${runs_root}"
