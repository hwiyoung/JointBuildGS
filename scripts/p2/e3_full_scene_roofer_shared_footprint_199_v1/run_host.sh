...
config_container="/workspace/JointBuildGS/configs/p2/e3_full_scene_roofer_shared_footprint_199_v1/run_v1.json"
condition_id="E3_GS_image"
task_name="P2-E3-FULL-SCENE-ROOFER-SHARED-FOOTPRINT-199-v1"
task_rel="phase-payloads/p2/e3_full_scene_roofer_shared_footprint_199_v1/${task_name}"
...
if [[ ! -f "${partial_root}/control/prepared_v1.json" ]]; then
  tools_run "python -B -m scripts.p2.e3_full_scene_roofer_shared_footprint_199_v1.run --config '${config_container}' prepare --output-root /task --artifact-root /artifacts/JointBuildGS --repo-root /workspace/JointBuildGS --scope full199"
fi
...
if [[ ! -f "${work}/fused_surface_receipt.json" ]]; then
  project_run python -B -m scripts.p2.e3_full_scene_roofer_shared_footprint_199_v1.run \
...
if [[ ! -f "${work}/classified_scene_receipt.json" ]]; then
  ...
  tools_run "python -B -m scripts.p2.e3_full_scene_roofer_shared_footprint_199_v1.run --config '${config_container}' verify-classified --output-root /task --condition-id '${condition_id}'"
fi
...
if [[ ! -f "${work}/roofer_terminal.json" ]]; then
  ...
  exit_code=$?
  set -e
  tools_run "python -B -m scripts.p2.e3_full_scene_roofer_shared_footprint_199_v1.run --config '${config_container}' record-roofer --output-root /task --condition-id '${condition_id}' --exit-code '${exit_code}' --runtime-seconds '$((SECONDS - start_seconds))'"
fi

tools_run "python -B -m scripts.p2.e3_full_scene_roofer_shared_footprint_199_v1.run --config '${config_container}' finalize --output-root /task"
mv -- "${partial_root}" "${final_root}"
echo "E3 shared-footprint Roofer complete: ${final_root}"
