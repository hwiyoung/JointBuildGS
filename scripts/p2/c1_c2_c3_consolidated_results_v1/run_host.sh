...
relative_root="phase-payloads/p2/c1_c2_c3_consolidated_results_v1/P2-C1-C2-C3-CONSOLIDATED-RESULTS-v1"
...
[[ ! -e "${final_root}" && ! -e "${partial_root}" ]] || { echo "add-once output namespace exists" >&2; exit 2; }
[[ "$(docker image inspect "${project_image}" --format '{{.Id}}')" == "${project_image}" ]] || { echo "project image identity mismatch" >&2; exit 2; }
...
docker run --rm --network none --cpus 4 --memory 32g --pids-limit 1024 \
  ...
  scripts/p2/c1_c2_c3_consolidated_results_v1/compose.py \
