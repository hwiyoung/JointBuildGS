...
완전판 v2 시도는 C1 표시 기준 LAS의 operation 이름을 잘못 적어 출력 생성 전에 중단됐고 해당 namespace는 보존했다. v3는 경로를 복구해 22행 source 판을 만들었다. v4는 계산 없이 같은 봉인 panel을 12행×8열로 재배열했다. v5는 terrain Z 극단값을 표시 축에서 제외했고, 최종 v6는 긴 설명문을 축 밖 하단으로 분리해 7행 축을 정상 크기로 복구했다. 원본 LAS와 Roofer output은 변경하지 않았다.
...
def run(source_root: Path, input_root: Path, diagnostic_root: Path, output_root: Path, source_commit: str) -> dict[str, Any]:
    index = _read_json(source_root / "qualitative/index_v6.json")
    ...
    source_material = [source_root / "qualitative/index_v6.json"] + [source_root / row["case_sheet"]["path"] for row in index["case_sheets"]]
    manifest = {
