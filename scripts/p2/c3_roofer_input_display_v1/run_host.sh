...
relative_root="phase-payloads/p2/c3_roofer_input_display_v1/P2-C3-ROOFER-INPUT-DISPLAY-RECOVERY-v1"
...
if [[ -n "$(git -C "${repo_root}" status --porcelain=v1 --untracked-files=all)" ]]; then
  echo "Roofer-input display requires a clean source checkout" >&2
  exit 2
fi
head_commit="$(git -C "${repo_root}" rev-parse HEAD)"
if [[ "${head_commit}" != "$(git -C "${repo_root}" rev-parse origin/main)" ]]; then
  echo "Roofer-input display requires HEAD=origin/main" >&2
  exit 2
fi
...
docker run --rm --network none \
  ...
  "${image}" python -B -m scripts.p2.c3_roofer_input_display_v1.render \
    --output-root "/artifacts/JointBuildGS/${relative_root}" \
    --artifact-root /artifacts/JointBuildGS

echo "C3 Roofer-input display complete: ${output_root}"
