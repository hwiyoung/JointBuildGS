...
RECEIPT="artifacts/manifests/handoffs/P2-W2C-C3-DEVELOPMENT-EVALUATION-CANDIDATE-RECOVERY-v1/100-accepted.json"
PACKET="docs/handoffs/P2_W2C_C3_DEVELOPMENT_EVALUATION_CANDIDATE_RECOVERY_v1.md"
...
[[ -f "${TASK_ROOT}/control/prepared_v1.json" ]]
[[ -f "${TASK_ROOT}/freeze/evaluation_units_v1.tsv" ]]

while IFS=$'\t' read -r unit_id unit_key city_rel; do
  [[ "${unit_id}" == "operation_unit_id" ]] && continue
  work="${TASK_ROOT}/g2/${unit_key}"
  [[ -f "${work}/stdout.txt" && -f "${work}/stderr.txt" && -f "${work}/exit_code.txt" ]]
