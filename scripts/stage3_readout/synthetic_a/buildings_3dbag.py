...
def classify_roof_type_from_faces(faces_with_labels):
    """Classify roof type from GT face data (not estimation).

    Counts RoofSurface faces and checks their normals.
    Uses data directly from CityJSON GT — no estimation involved.
    """
    roof_normals = []
    for pts, label, stype in faces_with_labels:
        if label == 1:  # Roof
            n = _face_normal_3d(pts)
            roof_normals.append(n)

    n_roof = len(roof_normals)
    if n_roof == 0:
        return 'unknown'

    if n_roof == 1:
        # Check if flat (normal ~vertical in 3D BAG Z-up) or shed (angled)
        z_abs = abs(roof_normals[0][2])
        return 'flat' if z_abs > 0.95 else 'shed'
    elif n_roof == 2:
        return 'gable'
    elif n_roof == 3:
        return 'tri-slope'
...
def parse_tile(tile_path, min_area=10.0, max_faces=200):
    ...
    for obj_name, obj in data.get('CityObjects', {}).items():
        ...
        # Classify roof type from GT geometry (before COLMAP conversion)
        roof_type_detailed = classify_roof_type_from_faces(all_faces)

        buildings.append({
            'id': len(buildings),
            'name': obj_name,
            'type': roof_type_detailed,
            'vertices': V,
            'faces': faces_list,
