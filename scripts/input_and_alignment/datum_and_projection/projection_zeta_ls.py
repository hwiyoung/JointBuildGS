...
from src.geospatial.projection_datum import describe_projection_config, load_projection_config, projection_geoid_m  # noqa: E402
...
def update_config(zeta_hat: float, ci_half: float) -> None:
    ...
    cfg["a1_zeta_ls"] = {
        ...
        "updated_by": "scripts/input_and_alignment/datum_and_projection/projection_zeta_ls.py --update-config",
    }
...
def write_versions(zeta0: float) -> None:
    ...
    text = "\n".join(
        [
            f"run_id: {RUN_ID}",
            f"git_head: {git_head()}",
            "command: python3 scripts/input_and_alignment/datum_and_projection/projection_zeta_ls.py --zeta0 48.0 --update-config",
