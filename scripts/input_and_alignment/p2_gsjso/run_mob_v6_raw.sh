...
# Launch: setsid nohup bash scripts/input_and_alignment/p2_gsjso/run_mob_v6_raw.sh > results/tum_transfer/mob/v6_raw.log 2>&1 &
...
for c in sparse dense acmp lidar; do
  ...
  docker run --rm --user "$(id -u):$(id -g)" -v "$REPO":/workspace/JointBuildGS -w /workspace/JointBuildGS "$TOOLS" \
    python3 scripts/input_and_alignment/p2_gsjso/tum_mob_raw_to_npz.py --cloud "$c" \
