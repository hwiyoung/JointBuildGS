...
def write_versions() -> None:
    ...
    text = "\n".join(
        [
            f"run_id: {RUN_ID}",
            f"git_head: {git_head()}",
            "command: python3 scripts/input_and_alignment/p2_gsjso/projection_datum_unitcheck.py",
...
def write_report(rows: list[dict[str, object]]) -> None:
    ...
    lines.extend(
        [
            ...
            "",
            "- Root file `원격발주_투영fix·LS정합·재게이트·재계산체인_레시피감사_20260702.md` was not present in this checkout; this A0 follows `CLAUDE.md`, `docs/experiments/input-and-alignment/projection_datum/reports/projection_geoid_rootcause.md`, and the retracted `docs/archive/projection_gate/v1/reports/projection_gate.md` note.",
            "",
            "## 판정 필요 지점",
            "",
