...
# Launch: setsid nohup bash scripts/input_and_alignment/p2_gsjso/run_mob_v6_protect.sh > results/tum_transfer/mob/v6_protect.log 2>&1 &
...
for name in $ARMS; do
  od="$OUT/$name"; mkdir -p "$od"
  {
    ...
    grep -E '^(seed_protect|init_pointcloud|init_pointcloud_mode|sem_detach_geometry|w_sem|w_structure|w_mutual|w_depth|w_normal|max_iter|grow_grad2d|refine_every|refine_stop_iter):' "configs/tum_mob/$name.yaml"
  } > "$od/versions.txt"
done
