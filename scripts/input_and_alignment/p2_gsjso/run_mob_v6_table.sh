...
# Usage: bash scripts/input_and_alignment/p2_gsjso/run_mob_v6_table.sh [matched|orig]
...
docker run --rm --user "$(id -u):$(id -g)" -v "$REPO":/workspace/JointBuildGS -w /workspace/JointBuildGS "$TOOLS" \
  python3 scripts/input_and_alignment/p2_gsjso/tum_mob_ref_rms.py --arms $ARMS \
...
python3 scripts/input_and_alignment/p2_gsjso/tum_mob_v6_table.py --tag "$TAG"
