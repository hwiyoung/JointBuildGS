...
# PREREQ: depth/normal maps staged at data_geoidfix/stereo (scripts/input_and_alignment/tum_transfer/prior_full_stereo.sh).
# Launch: setsid nohup bash scripts/input_and_alignment/tum_transfer/run_prior_full.sh > results/tum_transfer/mob/prior_full.log 2>&1 &
...
for name in $ARMS; do
  od="$OUT/$name"; mkdir -p "$od"
  {
    ...
    grep -E '^(seed_protect|init_pointcloud|sem_detach_geometry|w_sem|w_nc|w_depth|w_normal|depth_schedule|depth_warmup|w_structure|structure_grouping|structure_min_group|max_iter|grow_grad2d|refine_stop_iter):' "configs/input_and_alignment/tum_mob/$name.yaml"
  } > "$od/versions.txt"
done
