...
# Launch detached:  setsid nohup bash scripts/input_and_alignment/tum_transfer/run_mob_v6.sh > results/tum_transfer/mob/v6.log 2>&1 &
...
for name in $CONFIGS; do
  od="$OUT/$name"; mkdir -p "$od"
  {
    ...
    echo "config=configs/input_and_alignment/tum_mob/$name.yaml"
    case "$name" in
    ...
    grep -E '^(sem_detach_geometry|w_sem|w_structure|w_mutual|w_nc|w_depth|w_normal|max_iter|refine_every|grow_grad2d|init_pointcloud|init_pointcloud_mode):' "configs/input_and_alignment/tum_mob/$name.yaml"
  } > "$od/versions.txt"
done
...
if [ ! -f "$OUT/baselines.json" ]; then
  ...
  python3 scripts/input_and_alignment/tum_transfer/tum_mob_baselines.py > "$OUT/baselines.log" 2>&1 || \
