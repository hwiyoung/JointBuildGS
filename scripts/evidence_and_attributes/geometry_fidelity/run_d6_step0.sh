...
for arm in gs_d4_dense gs_d4_acmp; do
  for t in 4906969 42364659 4906972; do
    ...
    echo "  smrf $arm/$t"
    $TOOLS python3 scripts/input_and_alignment/tum_transfer/_mob_prep_las.py \
...
$TOOLS python3 scripts/evidence_and_attributes/geometry_fidelity/d6_overseg_diag.py

# ---- 3) component (c): Roofer epsilon / min-points sweep ----
echo "[3] d6_roofer_sweep.py (Roofer threshold sweep)"
python3 scripts/evidence_and_attributes/geometry_fidelity/d6_roofer_sweep.py
...
$TOOLS python3 scripts/evidence_and_attributes/geometry_fidelity/d6_density_match.py
echo "[4b] d6_densmatch_roofer.py (Roofer on density-matched clouds)  [host]"
python3 scripts/evidence_and_attributes/geometry_fidelity/d6_densmatch_roofer.py

# ---- 5) figures (clean; supersedes any in-diag scatter) ----
echo "[5] d6_figs.py (profile + roughness-dist + facets-vs-epsilon)  [p0-tools]"
$TOOLS python3 scripts/evidence_and_attributes/geometry_fidelity/d6_figs.py
