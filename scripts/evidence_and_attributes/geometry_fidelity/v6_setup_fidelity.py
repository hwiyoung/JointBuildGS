    ...
    def grep(path, pat):
        ...
        for ln in Path(path).read_text().splitlines():
            if re.search(pat, ln):
                out.append(ln.strip())
        return out
    gs = REPO / "scripts/input_and_alignment/tum_transfer/run_mob_v6.sh"
    rw = REPO / "scripts/input_and_alignment/tum_transfer/run_mob_v6_raw.sh"
    prep = REPO / "scripts/input_and_alignment/tum_transfer/_mob_prep_las.py"
