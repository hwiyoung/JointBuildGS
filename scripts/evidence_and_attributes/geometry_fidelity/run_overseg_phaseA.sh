...
smooth() { # $1=in $2=out $3=cell $4=win $5=npass(optional)
  docker run --rm --user "$U" -v "$REPO":/workspace/JointBuildGS -w /workspace/JointBuildGS "$TOOLS" \
    python3 scripts/evidence_and_attributes/geometry_fidelity/overseg_smooth.py --in "/workspace/JointBuildGS/$1" --out "/workspace/JointBuildGS/$2" --cell "$3" --win "$4" --npass "${5:-1}"
}
