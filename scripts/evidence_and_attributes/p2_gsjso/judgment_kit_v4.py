...
def write_versions(manifest: list[dict[str, object]], manual_missing: bool) -> None:
    ...
    lines = [
        f"run_id: {RUN_ID}",
        f"git_head: {git_head()}",
        "command: docker run --rm --user $(id -u):$(id -g) -v $PWD:/workspace/JointBuildGS -w /workspace/JointBuildGS jointbuildgs-p0-tools:t0 python3 scripts/evidence_and_attributes/p2_gsjso/judgment_kit_v4.py",
