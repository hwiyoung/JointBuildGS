...
def write_versions(path: Path, repo: Path, args, fps: dict[str, tuple[str, str]], counts: dict[str, object]) -> None:
    ...
    lines = [
        ...
        'run_command: docker run --rm --user "$(id -u):$(id -g)" -v "$PWD":/workspace/JointBuildGS -w /workspace/JointBuildGS jointbuildgs-p0-tools:t0 python3 scripts/evidence_and_attributes/p2_gsjso/attr_outcome_regression_v1.py',
...
def source_fingerprints(repo: Path, args) -> dict[str, tuple[str, str]]:
    paths = {
        ...
        "basis_document": repo / "기준문서_방법론·모집단·비교설계_v1.md",
    }
    out = {}
