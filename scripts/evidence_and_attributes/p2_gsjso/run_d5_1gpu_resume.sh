...
# Launch: setsid nohup bash scripts/evidence_and_attributes/p2_gsjso/run_d5_1gpu_resume.sh > results/tum_transfer/mob/d5_1gpu.log 2>&1 < /dev/null &
set -u
cd "$(dirname "$0")/../../.." || exit 1
...
echo "[$(date '+%T')] EVAL gssem"; python3 scripts/input_and_alignment/p2_gsjso/tum_mob_eval.py --configs $CFG --geojson "$GEOJSON" --classifier gssem --out "$OUT/eval_d5_gssem.json" > "$OUT/eval_d5_gssem.log" 2>&1
echo "[$(date '+%T')] EVAL smrf";  python3 scripts/input_and_alignment/p2_gsjso/tum_mob_eval.py --configs $CFG --geojson "$GEOJSON" --classifier smrf  --out "$OUT/eval_d5_smrf.json"  > "$OUT/eval_d5_smrf.log"  2>&1
