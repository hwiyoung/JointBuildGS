...
# Launch: setsid nohup bash scripts/evidence_and_attributes/p2_gsjso/run_d5_gssem_requal.sh > results/tum_transfer/mob/d5_requal.log 2>&1 < /dev/null &
set -u
cd "$(dirname "$0")/../../.." || exit 1
...
$TOOLS python3 scripts/input_and_alignment/p2_gsjso/tum_mob_ref_rms.py --arms $ARMS --out "$A/ref_rms_d5_smrf.csv" > "$BK/rms_d5_smrf.log" 2>&1
...
python3 scripts/evidence_and_attributes/p2_gsjso/gssem_requal_numbers.py smrf "$BK/numbers_d5_smrf.json" D5 || { echo fail>"$BK/PART2_FAIL"; exit 3; }
...
python3 scripts/input_and_alignment/p2_gsjso/tum_mob_eval.py --configs $CFG --geojson "$GEOJSON" --classifier gssem --out "$OUT/eval_d5_gssem.json" > "$BK/eval_d5_gssem.log" 2>&1

# 3) GSSEM RMS + snapshot
echo "[$(date '+%T')] RMS gssem (regenerated gssem .las)"
$TOOLS python3 scripts/input_and_alignment/p2_gsjso/tum_mob_ref_rms.py --arms $ARMS --out "$A/ref_rms_d5_gssem.csv" > "$BK/rms_d5_gssem.log" 2>&1
python3 scripts/evidence_and_attributes/p2_gsjso/gssem_requal_numbers.py gssem "$BK/numbers_d5_gssem.json" D5
...
python3 scripts/evidence_and_attributes/p2_gsjso/d5_cp_table.py > "$BK/d5_cp_table.log" 2>&1
