...
run_tools python3 scripts/evaluation/quality_score/qs_cheap_refine_pilot.py prepare \
  > "$RUN/logs/prepare.log" 2>&1

write_status "running" "MLS default refinement"
run_tools python3 scripts/evidence_and_attributes/geometry_fidelity/overseg_smooth.py \
...
run_tools python3 scripts/evaluation/quality_score/qs_cheap_refine_pilot.py finalize \
