...
# Launch: setsid nohup bash scripts/evidence_and_attributes/diagnostic_tables/run_d5_1gpu_resume.sh > results/tum_transfer/mob/d5_1gpu.log 2>&1 < /dev/null &
set -u
cd "$(dirname "$0")/../../.." || exit 1
...
train_one(){  # $1 = config name
  ...
  { echo "run=$1 stamped=$(date '+%F %T') git=$COMMIT branch=$BR (1-GPU resume, NVIDIA_VISIBLE_DEVICES=$GPU=GPU-fab8)"
    ...
    grep -E '^(w_structure_cp|structure_warmup|w_depth|w_normal|w_structure_na|max_iter):' "configs/input_and_alignment/tum_mob/$1.yaml"
  } > "$od/versions.txt"
...
for n in $ALL; do
  ...
  docker compose run --rm -T -e NVIDIA_VISIBLE_DEVICES=$GPU dev python scripts/stage3_readout/tum_mob_tsdf_extract.py \
...
echo "[$(date '+%T')] EVAL gssem"; python3 scripts/input_and_alignment/tum_transfer/tum_mob_eval.py --configs $CFG --geojson "$GEOJSON" --classifier gssem --out "$OUT/eval_d5_gssem.json" > "$OUT/eval_d5_gssem.log" 2>&1
echo "[$(date '+%T')] EVAL smrf";  python3 scripts/input_and_alignment/tum_transfer/tum_mob_eval.py --configs $CFG --geojson "$GEOJSON" --classifier smrf  --out "$OUT/eval_d5_smrf.json"  > "$OUT/eval_d5_smrf.log"  2>&1
