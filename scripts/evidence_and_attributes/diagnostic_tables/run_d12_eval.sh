...
# Launch: setsid nohup bash scripts/evidence_and_attributes/diagnostic_tables/run_d12_eval.sh [targets_file] > results/tum_transfer/mob/d12.log 2>&1 &
set -u
cd "$(dirname "$0")/../../.." || exit 1
...
if [ ! -f "$BASE" ]; then
  $TOOLS python3 scripts/input_and_alignment/tum_transfer/tum_mob_baselines.py --gml $GML \
...
python3 scripts/input_and_alignment/tum_transfer/tum_mob_eval.py --configs $CFG_GS --geojson "$GEOJSON" \
...
python3 scripts/input_and_alignment/tum_transfer/tum_mob_eval.py --configs raw_lidar="$OUT/raw/raw_lidar.npz" --geojson "$GEOJSON" \
