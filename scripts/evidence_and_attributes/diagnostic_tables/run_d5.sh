...
# Spec/pre-registration: docs/research/preregistration/joint_optimization/P2_D5_cp_ablation_사양_사전등록_20260626.md. EPSG:25832. branch feature/p2-prior-full.
...
# Launch: setsid nohup bash scripts/evidence_and_attributes/diagnostic_tables/run_d5.sh > results/tum_transfer/mob/d5.log 2>&1 &
set -u
cd "$(dirname "$0")/../../.." || exit 1
...
for name in $ARMS; do
  od="$OUT/$name"; mkdir -p "$od"
  {
    ...
    grep -E '^(seed_protect|init_pointcloud|sem_detach_geometry|w_photo|w_sem|w_nc|w_depth|w_normal|depth_schedule|depth_warmup|w_structure|w_structure_na|w_structure_cp|structure_grouping|structure_warmup|structure_min_group|max_iter|grow_grad2d|refine_stop_iter):' "configs/input_and_alignment/tum_mob/$name.yaml"
  } > "$od/versions.txt"
done
...
python3 scripts/input_and_alignment/tum_transfer/tum_mob_eval.py --configs $CFG --geojson "$GEOJSON" \
