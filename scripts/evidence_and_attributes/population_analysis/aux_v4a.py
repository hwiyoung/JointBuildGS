...
from scripts.evidence_and_attributes.population_analysis import population_aux_v3 as v3  # noqa: E402
...
ROOT_SPEC = REPO / "docs/research/methodology/기준문서_방법론·모집단·비교설계_v1.md"
GEOID_M = 45.700
UNCERTAINTY_M = 0.05
GEOM_ALLOW_M = 2.0
MASK_ALLOW_M = 0.3
...
def write_report(rows: list[dict[str, object]], cross: list[dict[str, object]], metrics: dict[str, object]) -> None:
    ...
    spec_note = (
        "`docs/research/methodology/기준문서_방법론·모집단·비교설계_v1.md`는 현재 checkout 루트에서 발견되지 않아 직접 인용하지 못했다."
        if not ROOT_SPEC.exists()
        else "`docs/research/methodology/기준문서_방법론·모집단·비교설계_v1.md` §1.6을 입력 기준으로 확인했다."
    )
    lines = [
...
def write_versions(metrics: dict[str, object]) -> None:
    ...
    lines = [
        f"run_id: {RUN_ID}",
        f"git_head: {git_head()}",
        "command: python scripts/evidence_and_attributes/population_analysis/aux_v4a.py",
