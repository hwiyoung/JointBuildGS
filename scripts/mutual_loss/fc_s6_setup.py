...
STAGE3_CMD="python scripts/mutual_loss/fc_s6_stage3_metric_v1_eval.py --run-name $ARM --config $CONFIG_PATH --checkpoint $CKPT_PATH --rendered-evidence-root $RENDERED_EVIDENCE_DIR --out-csv $METRICS_CSV --split-summary-csv $SPLIT_CSV"
...
case "$PHASE" in
  ...
  phase3)
    ...
    python scripts/mutual_loss/fc_s6_setup.py --materialize-phase3 "$(cat "$SELECTED")"
...
REPO_ROOT="$(cd "$(dirname "$0")/../../.." && pwd)"
cd "$REPO_ROOT"
python scripts/mutual_loss/fc_s6_collect_results.py
"""
    )
    ...
    for arm in arms():
        ...
        manifest_rows.append({
            ...
            "stage3_evaluation_command": f"python scripts/mutual_loss/fc_s6_stage3_metric_v1_eval.py --run-name {arm.name} --config configs/mutual_loss/fc_screening/fc_s6/{arm.name}.yaml --checkpoint {out_dir}/ckpt/final.pt --rendered-evidence-root {out_dir}/rendered_evidence --out-csv {metrics} --split-summary-csv {split}",
