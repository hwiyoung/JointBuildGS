...
echo "[fc-s6d2] eval=python scripts/mutual_loss/fc_s6_stage3_metric_v1_eval.py --run-name $ARM --config $CONFIG --checkpoint $CHECKPOINT --rendered-evidence-root $EVIDENCE --out-csv $METRICS --split-summary-csv $SPLIT --win-loss-csv $WINLOSS"
set +e
...
python scripts/mutual_loss/fc_s6d2_directional_screening.py --post-eval || true
echo "finished_at: $(date -Is)" >> "$JOB_RECORD"
exit "$EVAL_STATUS"
"""
    )
...
"""
    ...
    manifest = [
        ...
        f"- stage3_evaluation_command: `python scripts/mutual_loss/fc_s6_stage3_metric_v1_eval.py --run-name A8_v2_geo --config {rel(cfg_path)} --checkpoint {rel(checkpoint)} --rendered-evidence-root {rel(evidence)} --out-csv {rel(metrics)} --split-summary-csv {rel(split)} --win-loss-csv {rel(win_loss)}`",
