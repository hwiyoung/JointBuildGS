...
from scripts.mutual_loss.fc_s6d_directionality_setup import (  # noqa: E402
    A8_CKPT,
    A8_CONFIG,
    add_grads,
    compute_base_loss,
...
stage3_evaluation_command: python scripts/mutual_loss/fc_s6_stage3_metric_v1_eval.py --run-name $ARM --config $CONFIG --checkpoint $CHECKPOINT --rendered-evidence-root $EVIDENCE --out-csv $METRICS --split-summary-csv $SPLIT --win-loss-csv $WINLOSS
...
echo "[fc-s6e] eval=python scripts/mutual_loss/fc_s6_stage3_metric_v1_eval.py --run-name $ARM --config $CONFIG --checkpoint $CHECKPOINT --rendered-evidence-root $EVIDENCE --out-csv $METRICS --split-summary-csv $SPLIT --win-loss-csv $WINLOSS"
set +e
python scripts/mutual_loss/fc_s6_stage3_metric_v1_eval.py \\
    --run-name "$ARM" \\
    --config "$CONFIG" \\
...
python scripts/mutual_loss/fc_s6e_joint_screening.py --post-eval || true
echo "finished_at: $(date -Is)" >> "$JOB_RECORD"
exit "$EVAL_STATUS"
"""
    )
...
"""
    ...
    manifest = [
        ...
        f"- stage3_evaluation_command: `python scripts/mutual_loss/fc_s6_stage3_metric_v1_eval.py --run-name {JOINT_RUN} --config {rel(cfg_path)} --checkpoint {rel(checkpoint)} --rendered-evidence-root {rel(evidence)} --out-csv {rel(metrics)} --split-summary-csv {rel(split)} --win-loss-csv {rel(win_loss)}`",
