...
def source_fingerprints(repo: Path, status_used: Path) -> dict[str, tuple[str, str]]:
    paths = {
        ...
        "w3_repeatability_building_status": repo / "docs/evidence/p0-audit/w3-quality-integration/tables/W3_2b_roofer_repeatability_building_status.csv",
...
def write_versions(path: Path, args, provenance: dict[str, str], rows: list[dict[str, object]], fps: dict[str, tuple[str, str]]) -> None:
    ...
    lines = [
        ...
        'run_command: docker run --rm --user "$(id -u):$(id -g)" -v "$PWD":/workspace/JointBuildGS -w /workspace/JointBuildGS jointbuildgs-p0-tools:t0 python3 scripts/e5_c001/pointcloud_attributes_v1_1.py',
        "",
        "height_datum:",
    ...
    lines += [
        ...
        "",
        "outputs:",
        "  docs/evidence/archive/pointcloud_attributes/v1_1/tables/pointcloud_attributes_v1_1.csv",
...
def build_v11_section(
    ...
    lines: list[str] = [
        ...
        f"- v1의 +48.0은 `scripts/input_and_alignment/tum_transfer/tum_mob_raw_to_npz.py`와 `docs/experiments/input-and-alignment/pointcloud_attributes/reports/W_pointcloud_attributes.md`의 ACMP/LiDAR raw-arm 관행값이다. v1.1에서는 orthometric ACMP/ALS에 +{HEIGHT_CONSTANT_M:.3f} m를 썼다.",
        f"- v1의 +48.165는 `scripts/e5_c001/pointcloud_attributes_v1.py`의 `GEOID_MED_M`으로, 참조 LoD2 지붕 Z를 raw-arm 높이와 비교할 때만 더했던 값이다.",
    ...
    lines += [
        ...
        "- 반복성 기록 위치: `docs/evidence/p0-audit/w3-quality-integration/reports/W3_2b_roofer_repeatability.md`, `docs/evidence/p0-audit/w3-quality-integration/tables/W3_2b_roofer_repeatability_building_status.csv`.",
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--v1-csv", default="docs/evidence/archive/pointcloud_attributes/v1/tables/pointcloud_attributes_v1.csv")
    ap.add_argument("--out-csv", default="docs/evidence/archive/pointcloud_attributes/v1_1/tables/pointcloud_attributes_v1_1.csv")
