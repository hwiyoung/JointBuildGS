...
TARGET_PATHS = [
    Path("configs/input_and_alignment/p2_gsjso/seed_prep_acmp.json"),
    Path("scripts/input_and_alignment/p2_gsjso/tum_mob_seed_prep.sh"),
    Path("scripts/input_and_alignment/p2_gsjso/tum_mob_raw_to_npz.py"),
    Path("scripts/input_and_alignment/p2_gsjso/seed_depth_bands.py"),
    Path("scripts/input_and_alignment/p2_gsjso/seed_material_audit.py"),
...
def write_versions(path: Path, run_id: str, gate_doc: Path, hit_count: int) -> None:
    ...
    lines = [
        ...
        f"script=scripts/e5_c001/p2_gsjso/e5_seed_constant_gate.py sha256={sha256(Path(__file__))}",
        "",
        "new_execution_paths:",
    ]
    for rel in TARGET_PATHS:
