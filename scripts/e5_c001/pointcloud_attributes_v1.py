...
def load_ref_invalid(repo: Path) -> dict[str, str]:
    ...
    for rel in (
        "docs/evidence/p0-audit/w3-quality-integration/tables/W3_2c_canonical_paired_status.csv",
        "docs/evidence/p0-audit/w2-reconstruction-audit/tables/W2_1c_paired_status.csv",
    ):
        p = repo / rel
        if not p.exists():
            continue
...
def write_versions(path: Path, args, provenance: dict[str, str], rows: list[dict]) -> None:
    ...
    lines = [
        ...
        'run_command: docker run --rm --user "$(id -u):$(id -g)" -v "$PWD":/workspace/JointBuildGS -w /workspace/JointBuildGS jointbuildgs-p0-tools:t0 python3 scripts/e5_c001/pointcloud_attributes_v1.py',
    ...
    lines += [
        "",
        "outputs:",
        "  docs/evidence/archive/pointcloud_attributes/v1/tables/pointcloud_attributes_v1.csv",
...
def build_argparser() -> argparse.ArgumentParser:
    ...
    ap.add_argument("--out-csv", default="docs/evidence/archive/pointcloud_attributes/v1/tables/pointcloud_attributes_v1.csv")
