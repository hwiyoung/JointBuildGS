...
# Launch: setsid nohup bash scripts/quality_score/p2_gsjso/run_overnight.sh > results/tum_transfer/mob/overnight.log 2>&1 < /dev/null &
cd "$(dirname "$0")/../../.." || exit 1
...
{
  $TOOLS python3 scripts/evidence_and_attributes/p2_gsjso/d12_metric_final.py --targets-file "$OUT/d12_targets_79.txt" \
  ...
  git add docs/experiments/evaluation/w_d12_metric/reports/W_D12_metric_final.md scripts/evidence_and_attributes/p2_gsjso/d12_metric_final.py \
...
{
  # Step0 — bucket labels
  $TOOLS python3 scripts/evidence_and_attributes/p2_gsjso/d12_buckets.py > "$OUT/overnight_logs/taskB_buckets.log" 2>&1 \
  ...
  if [ ! -f "$OUT/overseg_lever/d12_buckets.csv" ]; then
    ...
    if [ ! -f "$BASE" ]; then
      $TOOLS python3 scripts/input_and_alignment/p2_gsjso/tum_mob_baselines.py \
    ...
    eval_one(){ # $1=arm $2=npz $3=classifier $4=targets $5=tag
      ...
      python3 scripts/input_and_alignment/p2_gsjso/tum_mob_eval.py --configs ${arm}=${npz} --geojson "$GEOJSON" \
    ...
    for arm in gs_seed_sparse gs_seed_dense gs_seed_acmp; do
      ...
      for ((i=0;i<NM;i+=CG)); do
        ...
        python3 scripts/input_and_alignment/p2_gsjso/tum_mob_eval.py --configs ${arm}=$OUT/tsdf_gen8.npz --geojson "$GEOJSON" \
    ...
    $TOOLS python3 scripts/quality_score/p2_gsjso/gen_8way_aggregate.py > "$OUT/overnight_logs/taskB_aggregate.log" 2>&1 \
      && log "aggregate OK" || issue "Task B gen_8way_aggregate.py failed"
    git add docs/experiments/evaluation/w_generation_8way/reports/W_generation_8way.md scripts/evidence_and_attributes/p2_gsjso/d12_buckets.py \
            scripts/quality_score/p2_gsjso/gen_8way_aggregate.py scripts/quality_score/p2_gsjso/run_overnight.sh 2>/dev/null
