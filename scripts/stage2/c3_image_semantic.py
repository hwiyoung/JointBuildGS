#!/usr/bin/env python3
"""CLI for exact-937 COLMAP-undistorted image-only semantic inference."""
...
def main() -> None:
    ...
    parser.add_argument("--cameras-bin", type=Path, required=True)
    parser.add_argument("--images-bin", type=Path, required=True)
    parser.add_argument("--geometric-depth-root", type=Path, required=True)
    ...
    result = produce(
        ...
        geometric_depth_root=args.geometric_depth_root,
