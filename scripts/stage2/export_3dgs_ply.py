...
Quaternion layout:
  - rot_0..3 = w,x,y,z, matching the INRIA 3DGS PLY convention.
    GaussianSplats3D's PLY parser also expects this convention before
    converting to its internal x,y,z,w texture layout.
"""
import argparse
...
def main():
    ...
    ap.add_argument('--third-log-scale', type=float, default=-13.815510557964274,
                    help='fallback log scale for flat 3rd axis when ckpt has only 2 scale dims; default is log(1e-6)')
    args = ap.parse_args()
    ...
    log_scales = sd['log_scales'].numpy().astype(np.float32)  # (N,2) or (N,3)
    ...
    if args.subsample and args.subsample < N:
        ...
        opacities_raw = opacities_raw[idx]
        sh0 = sh0[idx]
        shN = shN[idx]
    ...
    # Build flat 3rd scale. Current ckpts already store a near-zero 3rd scale.
    ...
    nn = np.maximum(nn, 1e-8)
    nx, ny, nz = nx / nn, ny / nn, nz / nn

    # INRIA stores f_rest channel-first: R coefficients, then G, then B.
    K = shN.shape[1]
    f_rest = shN.transpose(0, 2, 1).reshape(N, K * 3)

    # f_dc: first SH coefficient
    f_dc = sh0  # (N, 3)
