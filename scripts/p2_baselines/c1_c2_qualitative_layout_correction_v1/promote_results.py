...
REPORT_REL = "docs/experiments/p2/c1_c2_qualitative_layout_correction_v1/C1_C2_QUALITATIVE_LAYOUT_CORRECTION_v1.md"
TECHNICAL_REL = "artifacts/manifests/p2_baselines/c1_c2_qualitative_layout_correction_v1/technical_result_manifest_v1.json"
...
def promote(*, external_manifest: Path, repo_root: Path, source_commit: str, accepted_commit: str) -> dict[str, Any]:
    ...
    commands = [str(value).upper() for value in accepted.get("verification", {}).get("commands", [])]
    if (
        len(zero_tests) != 1
        or zero_tests[0].get("passed") != 0
        or zero_tests[0].get("failed") != 0
        or not any("WITHOUT --ARTIFACT-ROOT" in value and "ZERO" in value and "READ" in value and "HASH" in value for value in commands)
    ):
    ...
    report = f"""# C1/C2 qualitative layout correction v1
...
The prior blocked namespace and every prior packet, Return, receipt, report, table,
and manifest remain immutable. Final technical closure belongs to the separately
