...
> 작성=에이전트(관찰·산출까지). §5 **확정·판정=김휘영**. 결과는 본 문서가 아니라 `docs/W_D5.md`에 쓰고 §5에 대입한다.
> 학습 ~4h/arm · 학습 **승인=김휘영**. 관찰만, 판정 금지. 기반: [[W_D4]](docs/W_D4.md) · [[W_D4_precheck]](docs/W_D4_precheck.md).
...
- **산출물**: `docs/W_D5.md`(과분할/RMS/solid/생성 표 + 정성 그림 + 본 §5 대입). 한 커밋 "D5". results/ data는 gitignore(재생성).
