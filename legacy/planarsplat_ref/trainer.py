...
from utils.loss_util import normal_loss, metric_depth_loss, semantic_loss, normal_consistency_loss, mutual_loss, photo_loss, multi_view_consistency_loss
...
class PlanarSplatTrainRunner():
    def __init__(self, **kwargs):
        ...
        sbs = self.conf.get_float('dataset.scene_bounding_sphere', default=5.0)
        self.max_depth = loss_plane_conf.get_float('max_depth', default=2.0 * sbs)
        self.depth_scale = loss_plane_conf.get_float('depth_scale', default=1.0)
        # L_mutual (Phase 3-A)
        ...
        e_grav_list = loss_plane_conf.get_list('e_gravity', default=[0., -1., 0.])
        self.e_gravity = torch.tensor([float(x) for x in e_grav_list], device='cuda')
        self.e_gravity = self.e_gravity / self.e_gravity.norm()  # ensure unit vector
        # L_mvc: multi-view consistency
        self.lambda_mvc = loss_plane_conf.get_float('lambda_mvc', default=0.0)
    ...
    def train(self):
        ...
        for iter in range(self.start_iter, self.max_total_iters + 1):
            ...
            loss_plane_depth = metric_depth_loss(depth, view_info.mono_depth, valid_ray_mask, max_depth=self.max_depth, depth_scale=self.depth_scale)
            ...
            # ------------ L_mvc: multi-view consistency
            loss_mvc_value = 0.
            if self.lambda_mvc > 0 and self.ds_len > 1:
                # Pick a random reference view different from current
                from random import randint as _randint
                ref_idx = _randint(0, self.ds_len - 1)
                while ref_idx == view_info.index:
                    ref_idx = _randint(0, self.ds_len - 1)
                ref_view = self.dataset.view_info_list[ref_idx]
                ref_view.to_gpu()

                # Render reference view
                with torch.no_grad():
                    ref_allmap = self.net.planarSplat(ref_view, iter)
                ref_depth = ref_allmap[0:1].squeeze()  # (H, W)
                ref_normal_local = ref_allmap[2:5]
                ref_raster_w2c = ref_view.raster_cam_w2c
                ref_normal_global = (ref_normal_local.permute(1, 2, 0) @ (ref_raster_w2c[:3, :3].T))  # (H, W, 3)

                H_img, W_img = self.dataset.img_res
                loss_mvc = multi_view_consistency_loss(
                    depth.view(H_img, W_img),
                    normal_global.view(H_img, W_img, 3),
                    view_info.intrinsic, view_info.pose,
                    ref_depth, ref_normal_global,
                    ref_view.intrinsic, ref_view.pose,
                    H_img, W_img)
                loss_final += self.lambda_mvc * loss_mvc * decay
                loss_mvc_value = loss_mvc.detach().item()
                ref_view.to_cpu()
            ...
            with torch.no_grad():
                ...
                if iter % self.log_freq == 0:
                    ...
                    if self.enable_semantic and self.tb_writer is not None:
                        ...
                        if self.lambda_mvc > 0:
                            self.tb_writer.add_scalar('loss/mvc', loss_mvc_value, iter)
    ...
    def check_plane_visibility_cuda(self):   
        ...
        for iter in tqdm(range(self.ds_len)):
            ...
            loss_mono_depth = metric_depth_loss(depth, view_info.mono_depth, valid_ray_mask, depth_scale=self.depth_scale)
